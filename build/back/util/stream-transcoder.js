...
function Transcoder(source) {
        ...
        Transcoder.prototype._parseMetadata = function (child) {
                ...
                var _parseDuration = function _parseDuration(duration) {
                ...
                var metadataFilters = {
                        ...
                        'channels': {
                                ...
                                transform: function transform(r) {
                                        if (r == 'mono') return 1;
                                        if (r == 'stereo') return 2;else return parseInt(r);
                                }
                        },
                        'bitrate': {
                                match: /(\d+) (\w)?b\/s/i,
                ...
                var _applyFilters = function _applyFilters(data, filters) {
                ...
                var streamed = false;
                var streamline = '';
                var _endParse = function _endParse() {
                        if (streamline) {
                                current.streams.push(_applyFilters(streamline, metadataFilters));
                                streamline = '';
                        }
        ...
        Transcoder.prototype._exec = function (a) {
                ...
                var self = this;
                if ('string' == typeof this.source) a = ['-i', this.source].concat(a);else a = ['-i', '-'].concat(a);

                console.log('Spawning ffmpeg ' + a.join(' '));

                var child = spawn('avconv', a, {
                        cwd: os.tmpdir()
                });
                ...
                child.stdin.on('error', function (err) {
                        try {
                                if ('object' == (0, _typeof3.default)(self.source)) self.source.unpipe(this.stdin);
                        } catch (e) {
                                // Do nothing
                        }
                });
                ...
                child.on('exit', function (code) {
                        if (!code) self.emit('finish');else self.emit('error', new Error('FFmpeg error: ' + self.lastErrorLine));
                });
                ...
                */
                ...
                if ('object' == (0, _typeof3.default)(this.source)) this.source.pipe(child.stdin);
