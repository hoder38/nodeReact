...
exports.default = {
    getChart: function getChart(uid, user, session) {
        ...
        return (0, _mongoTool2.default)('find', _constants.RANKDB, { _id: id }).then(function (items) {
            if (items.length < 1) {
                return (0, _utility.handleError)(new _utility.HoError('rank cannot find!!!'));
            }
            ...
            return getName().then(function (itemName) {
                return findData().then(function (itemData) {
                    if (itemData.length < 1) {
                        return (0, _utility.handleError)(new _utility.HoError('no data!!!'));
                    }
                    var data = [];
                    var labels = [];
                    ...
                    var getUser = function getUser() {
                        return owner === _constants.RANK_LIMIT && !items[0].history ? (0, _mongoTool2.default)('find', items[0].type + 'Count', {
                            ...
                            if (items1.length < 1) {
                                return (0, _utility.handleError)(new _utility.HoError('rank cannot find user!!!'));
                            }
                            labels.push(user.username);
    ...
    newRow: function newRow(data) {
        ...
        if (!id) {
            return (0, _utility.handleError)(new _utility.HoError('item not vaild!!!'));
        }
        var date = new Date();
        ...
        return getItem().then(function (itemName) {
            return (0, _mongoTool2.default)('find', _constants.RANKDB, {
                ...
                if (items.length > 0) {
                    return (0, _utility.handleError)(new _utility.HoError('double rank!!!'));
                }
    ...
    delRow: function delRow(uid) {
        ...
        return (0, _mongoTool2.default)('find', _constants.RANKDB, { _id: id }, { limit: 1 }).then(function (items) {
            if (items.length < 1) {
                return (0, _utility.handleError)(new _utility.HoError('rank row does not exist!!!'));
            }
