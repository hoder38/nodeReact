...
router.put('/addTagUrl', function (req, res, next) {
    ...
    if (!url) {
        return (0, _utility.handleError)(new _utility.HoError('invalid tag url'), next);
    }
...
router.get('/media/setTime/:id/:type/:obj?/:pageToken?/:back(back)?', function (req, res, next) {
    ...
    if (id) {
        ...
        if (!id) {
            return (0, _utility.handleError)(new _utility.HoError('youtube is not vaild'), next);
        }
    } else {
        id = (0, _utility.isValidString)(req.params.id, 'uid');
        if (!id) {
            return (0, _utility.handleError)(new _utility.HoError('file is not vaild'), next);
        }
    ...
    if (!type) {
        return (0, _utility.handleError)(new _utility.HoError('type is not vaild'), next);
    }
            ...
            if (playlist) {
                if (playlist === 1) {
                    ...
                    return (0, _mongoTool2.default)('find', _constants.STORAGEDB, { _id: id }, { limit: 1 }).then(function (items1) {
                        if (items1.length < 1) {
                            return (0, _utility.handleError)(new _utility.HoError('cannot find external'));
                        }
                    ...
                    var playtype = null;
                    if (playlist === 3) {
                        playurl = 'http://www.iwatchme2u.com/vod-read-id-' + playlistId + '.html';
                        playtype = 'kubo';
                    } else if (playlist === 4) {
...
router.get('/media/record/:id/:time/:pId?', function (req, res, next) {
    ...
    if (!req.params.time.match(/^\d+(&\d+|\.\d+)?$/)) {
        return (0, _utility.handleError)(new _utility.HoError('timestamp is not vaild'), next);
    }
...
router.get('/media/more/:type(\\d+)/:page(\\d+)/:back(back)?', function (req, res, next) {
    ...
    switch (type) {
        ...
        case 4:
            ...
            break;
        default:
            return (0, _utility.handleError)(new _utility.HoError('unknown type'), next);
    }
    var sql = StorageTagTool.saveSql(Number(req.params.page), saveName, req.params.back, req.user, req.session);
    if (!sql) {
        return (0, _utility.handleError)(new _utility.HoError('query error'), next);
    }
    console.log(sql);
    if (sql.empty) {
        res.json({ itemList: [] });
...
router.put('/zipPassword/:uid', function (req, res, next) {
    ...
    if (!pwd) {
        return (0, _utility.handleError)(new _utility.HoError('password is not vaild'), next);
    }
    (0, _mongoTool2.default)('find', _constants.STORAGEDB, {
        ...
        if (items.length < 1) {
            return (0, _utility.handleError)(new _utility.HoError('zip can not be fund!!!'));
        }
