...
import GoogleApi, { userDrive, autoDoc, googleBackupDb, googleBackupWhole } from '../models/api-tool-google.js'
...
/*export const updateExternal = () => {
    if (UPDATE_EXTERNAL(ENV_TYPE)) {
        const loopUpdateExternal = () => {
            ...
            return completeMimeTag(1).catch(err => bgError(err, 'Loop updateExternal')).then(() => new Promise((resolve, reject) => setTimeout(() => resolve(), EXTERNAL_INTERVAL * 1000))).then(() => loopUpdateExternal());
        }
