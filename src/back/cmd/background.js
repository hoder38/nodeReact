...
export const updateStock = () => {
    if (UPDATE_STOCK(ENV_TYPE)) {
        const loopUpdateStock = () => {
            ...
            //let use_stock_list = [];
            const sd = new Date();
            const parseStockList = () => (sd.getDay() === 2 && sd.getHours() === 11) ? getStockListV2('twse', new Date().getFullYear(), new Date().getMonth() + 1).then(stocklist => {
                stocklist.forEach(i => stock_batch_list.push(i));
                //return updateStockList();
            }) : (sd.getDay() === 5 && sd.getHours() === 11) ? getStockListV2('usse', new Date().getFullYear(), new Date().getMonth() + 1).then(stocklist => {
                stocklist.forEach(i => stock_batch_list.push(i));
                //return updateStockList();
            }) : Promise.resolve();
