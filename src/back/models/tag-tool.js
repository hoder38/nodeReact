    ...
    return {
        ...
        getKuboQuery: function(search_arr, sortName, page) {
            ...
            if (type) {
                ...
                const url = searchWord ? `http://www.99kubo.tv/index.php?s=Vod-innersearch-q-${encodeURIComponent(searchWord)}-order-${sOrder}-page-${page}` : `http://www.99kubo.tv/vod-search-id-${type}-cid--tag--area-${country}-tag--year-${year}-wd--actor--order-${order}%20desc-p-${page}.html`;
                console.log(url);
                return url;
            } else {
                return false;
