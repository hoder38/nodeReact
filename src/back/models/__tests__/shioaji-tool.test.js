...
function buildPythonOutput({
  ...
  const profitLine = profit !== null ? `\n${typeof profit === 'string' ? profit : JSON.stringify(profit)}` : '';
  return `${preamble}\nstart result\n${available}\n${position}\n${order}\n${fillOrder}${profitLine}\n`;
}
...
describe('shioaji-tool.js', () => {
  ...
  // ═══════════════════════════════════════════════════════════
  // 14. profitData parsing and updateProfit
  // ═══════════════════════════════════════════════════════════
  describe('profitData parsing and updateProfit', () => {
    test('profit line present — profitData is set', async () => {
      const profitPayload = { items: [{ code: '2330', pnl: 1500, date: '2025-01-13' }] };
      const output = buildPythonOutput({ profit: profitPayload });
      mockExec.mockImplementation((cmd, cb) => cb(null, output));
      mockGetSuggestionData.mockReturnValue({});
      mockMongo.mockResolvedValue([]);
      ...
      expect(_getState().profitData).toEqual(profitPayload);
    });

    test('profit line absent — profitData is null', async () => {
      const output = buildPythonOutput(); // no profit field
      mockExec.mockImplementation((cmd, cb) => cb(null, output));
      mockGetSuggestionData.mockReturnValue({});
      mockMongo.mockResolvedValue([]);
      ...
      expect(_getState().profitData).toBeNull();
    });

    test('profitData reset to null on each getShioajiData call', async () => {
      // First call: with profit data
      _setState({ profitData: { items: [{ code: '2330', pnl: 999, date: '2025-01-01' }] } });
      const output = buildPythonOutput(); // no profit line
      mockExec.mockImplementation((cmd, cb) => cb(null, output));
      mockGetSuggestionData.mockReturnValue({});
      mockMongo.mockResolvedValue([]);
    ...
    test('invalid JSON profit line — profitData stays null', async () => {
      const output = buildPythonOutput({ profit: 'not-valid-json{{{' });
      mockExec.mockImplementation((cmd, cb) => cb(null, output));
      mockGetSuggestionData.mockReturnValue({});
      mockMongo.mockResolvedValue([]);
    ...
    test('updateProfit — no profitData — skips Mongo find for profit', async () => {
      const output = buildPythonOutput();
      mockExec.mockImplementation((cmd, cb) => cb(null, output));
      mockGetSuggestionData.mockReturnValue({});
      mockMongo.mockResolvedValue([]);
      ...
      // No Mongo calls with $in filter (the updateProfit path)
      const findCalls = mockMongo.mock.calls.filter(c => c[0] === 'find' && c[2] && c[2].index && c[2].index.$in);
      expect(findCalls.length).toBe(0);
    });

    test('updateProfit — empty items — skips Mongo find for profit', async () => {
      const output = buildPythonOutput({ profit: { items: [] } });
      mockExec.mockImplementation((cmd, cb) => cb(null, output));
      mockGetSuggestionData.mockReturnValue({});
      mockMongo.mockResolvedValue([]);
    ...
    test('updateProfit — new date > profit_date — applies pnl via $inc', async () => {
      const profitPayload = { items: [{ code: '2330', pnl: 1500, date: '2025-01-15' }] };
      // Pre-set profitData and trade=2 so submit block runs without exec
      const now = Math.round(Date.now() / 1000);
      _setState({ updateTime: { book: now, trade: 2 }, profitData: profitPayload });
      const origDate = global.Date;
      global.Date = class extends origDate { getHours() { return 20; } };

      mockGetSuggestionData.mockReturnValue({});
      const dbItem = { _id: 'id1', index: '2330', setype: 'twse', profit: 0 };
      mockMongo.mockImplementation((method, col, query) => {
        if (method === 'find' && query && query.index && query.index.$in) return Promise.resolve([dbItem]);
        if (method === 'find') return Promise.resolve([]);
        if (method === 'update') return Promise.resolve();
        return Promise.resolve();
      });
      ...
      const updateCalls = mockMongo.mock.calls.filter(c => c[0] === 'update' && c[3] && c[3].$inc);
      expect(updateCalls.length).toBeGreaterThan(0);
      expect(updateCalls[0][3].$inc.profit).toBe(1500);
      expect(updateCalls[0][3].$set.profit_date).toBe('2025-01-15');
    });

    test('updateProfit — date <= profit_date — does not apply pnl', async () => {
      const profitPayload = { items: [{ code: '2330', pnl: 1500, date: '2025-01-14' }] };
      const now = Math.round(Date.now() / 1000);
      _setState({ updateTime: { book: now, trade: 2 }, profitData: profitPayload });
      const origDate = global.Date;
      global.Date = class extends origDate { getHours() { return 20; } };
      ...
      mockGetSuggestionData.mockReturnValue({});
      const dbItem = { _id: 'id1', index: '2330', setype: 'twse', profit: 5000, profit_date: '2025-01-15' };
      mockMongo.mockImplementation((method, col, query) => {
        if (method === 'find' && query && query.index && query.index.$in) return Promise.resolve([dbItem]);
        if (method === 'find') return Promise.resolve([]);
        if (method === 'update') return Promise.resolve();
    ...
    test('updateProfit — multiple records same code — sums only newer pnl', async () => {
      const profitPayload = {
        items: [
          { code: '2330', pnl: 1000, date: '2025-01-13' },
          { code: '2330', pnl: 500, date: '2025-01-14' },
        ],
      };
      const now = Math.round(Date.now() / 1000);
      _setState({ updateTime: { book: now, trade: 2 }, profitData: profitPayload });
      ...
      mockGetSuggestionData.mockReturnValue({});
      const dbItem = { _id: 'id1', index: '2330', setype: 'twse', profit: 0, profit_date: '2025-01-13' };
      mockMongo.mockImplementation((method, col, query) => {
        if (method === 'find' && query && query.index && query.index.$in) return Promise.resolve([dbItem]);
        if (method === 'find') return Promise.resolve([]);
        if (method === 'update') return Promise.resolve();
      ...
      const updateCalls = mockMongo.mock.calls.filter(c => c[0] === 'update' && c[3] && c[3].$inc);
      expect(updateCalls.length).toBeGreaterThan(0);
      // Only 2025-01-14 record is newer than profit_date '2025-01-13'
      expect(updateCalls[0][3].$inc.profit).toBe(500);
      expect(updateCalls[0][3].$set.profit_date).toBe('2025-01-14');
    });

    test('updateProfit — multiple codes — updates each matching totaldb item', async () => {
      const profitPayload = {
        items: [
          { code: '2330', pnl: 1000, date: '2025-01-15' },
          { code: '2317', pnl: 200, date: '2025-01-15' },
        ],
      };
      const now = Math.round(Date.now() / 1000);
      _setState({ updateTime: { book: now, trade: 2 }, profitData: profitPayload });
      ...
      mockGetSuggestionData.mockReturnValue({});
      const dbItems = [
        { _id: 'id1', index: '2330', setype: 'twse', profit: 0 },
        { _id: 'id2', index: '2317', setype: 'twse', profit: 100 },
      ];
      mockMongo.mockImplementation((method, col, query) => {
        if (method === 'find' && query && query.index && query.index.$in) return Promise.resolve(dbItems);
        if (method === 'find') return Promise.resolve([]);
        if (method === 'update') return Promise.resolve();
        return Promise.resolve();
      });
