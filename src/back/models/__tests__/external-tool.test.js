 ...
 *   youtubePlaylist, getSingleId, saveSingle) + named (bilibiliVideoUrl,
 *   kuboVideoUrl, youtubeVideoUrl) + internal updateDocDate.
 *
...
jest.unstable_mockModule('../../constants.js', () => ({
  ...
  KUBO_TYPE: [
    ['動作片','喜劇片','愛情片','科幻片','恐怖片','劇情片','戰爭片','動畫片'],
    ['台灣劇'],
    ['綜藝'],
    ['動漫'],
  ],
...
// ---------------------------------------------------------------------------
let ExternalTool, bilibiliVideoUrl, kuboVideoUrl, youtubeVideoUrl;

beforeAll(async () => {
  const mod = await import('../external-tool.js');
  ExternalTool = mod.default;
  ({ bilibiliVideoUrl, kuboVideoUrl, youtubeVideoUrl } = mod);
});
...
describe('getSingleList', () => {
  ...
  // ----- bilibili -----
  describe('bilibili', () => {
    test('bilibili /list/ URL', async () => {
      const li = mkNode('li', null, [
        mkText('skip'),
        mkNode('div', null, [
          mkNode('div', 'l-l', [
            mkNode('a', null, [
              mkNode('img', null, [], { alt: 'TestVid', 'data-img': 'http://img.jpg' }),
            ], { href: '/video/av12345/' }),
          ]),
          mkNode('div', 'l-r', [
            mkNode('div', 'v-info', [
              mkNode('span', 'v-info-i gk', [
                mkNode('span', null, [], { number: '1000' }),
              ]),
            ]),
          ]),
        ]),
      ...
      mockParseDOM.mockReturnValue([mkText(''), mkNode('div', null, [li])]);
      const result = await ExternalTool.getSingleList('bilibili', 'https://www.bilibili.com/list/test');
      expect(result).toHaveLength(1);
      expect(result[0].id).toBe('av12345');
      expect(result[0].name).toBe('TestVid');
    });

    test('bilibili www URL (not list) — success', async () => {
      mockApi.mockResolvedValue({
        message: 'success',
        result: {
          list: [{
      ...
      const result = await ExternalTool.getSingleList('bilibili', 'https://www.bilibili.com/bangumi/');
      expect(result).toHaveLength(1);
      expect(result[0].id).toBe('s1');
      expect(result[0].tags).toContain('animation');
    });

    test('bilibili www URL — fail', async () => {
      mockApi.mockResolvedValue({ message: 'error' });
      await expect(ExternalTool.getSingleList('bilibili', 'https://www.bilibili.com/bangumi/'))
        .rejects.toThrow('bilibili api fail');
    });

    test('bilibili search URL — html with video matrix', async () => {
      const videoLi = mkNode('li', 'video matrix', [
        mkNode('a', null, [
          mkNode('div', 'img', [
            mkNode('img', null, [], { 'data-src': '//img.jpg' }),
          ]),
        ], { href: '/video/av99/', title: 'SearchVid' }),
        mkNode('div', 'info', [
          mkNode('div', 'tags', [
            mkNode('span', 'so-icon watch-num', [mkText('500')]),
          ]),
        ]),
      ]);
      mockApi.mockResolvedValue({ code: 0, html: '<div></div>' });
      mockParseDOM.mockReturnValue([videoLi]);
      const result = await ExternalTool.getSingleList('bilibili', 'http://search.bilibili.com/q=test');
      expect(result).toHaveLength(1);
      expect(result[0].id).toBe('av99');
    });

    test('bilibili search URL — html with synthetical fallback', async () => {
      const synthLi = mkNode('li', 'synthetical', [
        mkNode('div', 'left-img', [
          mkText('skip'),
          mkNode('a', null, [
            mkNode('img', null, [], { 'data-src': '//img2.jpg' }),
          ], { href: '/anime/88/', title: 'SynthVid' }),
        ]),
      ]);
      mockApi.mockResolvedValue({ code: 0, html: '<div></div>' });
      mockParseDOM.mockReturnValue([synthLi]);
      const result = await ExternalTool.getSingleList('bilibili', 'http://search.bilibili.com/q=test');
      expect(result).toHaveLength(1);
    ...
    test('bilibili search — no html', async () => {
      mockApi.mockResolvedValue({ code: 0 });
      const result = await ExternalTool.getSingleList('bilibili', 'http://search.bilibili.com/q=test');
      expect(result).toEqual([]);
    });

    test('bilibili search — code not 0 or 1', async () => {
      mockApi.mockResolvedValue({ code: 2 });
      await expect(ExternalTool.getSingleList('bilibili', 'http://search.bilibili.com/q=test'))
        .rejects.toThrow('bilibili api fail');
    });

    test('bilibili search — null response', async () => {
  ...
  // ----- kubo -----
  describe('kubo', () => {
    test('kubo with main div — type_id 1 (movie)', async () => {
      const img = mkNode('img', null, [], { alt: 'KuboMovie', 'data-original': 'http://k.jpg' });
      const li = mkNode('li', null, [
        mkNode('a', null, [img], { href: '/vod-123.html' }),
        mkNode('p', null, [mkText('主演：')]),
        mkNode('p', null, [mkText('地區/年份：中國/2020')]),
        mkNode('p', null, [mkText('月熱度：999')]),
        mkNode('p', null, [mkText('更新：2020-05-01 extra')]),
      ]);
      ...
      mockApi.mockResolvedValue('<html></html>');
      const result = await ExternalTool.getSingleList('kubo', 'http://kubo.tv/vod-search-id-1-page-1.html');
      expect(result).toHaveLength(1);
      expect(result[0].name).toBe('KuboMovie');
      expect(result[0].count).toBe(999);
    });

    test('kubo with main div — no type_id → error', async () => {
      const body = mkNode('body', null, [mkNode('div', 'main', [])]);
      mockParseDOM.mockReturnValue([mkNode('html', null, [body])]);
      mockApi.mockResolvedValue('');
      await expect(ExternalTool.getSingleList('kubo', 'http://kubo.tv/no-id'))
        .rejects.toThrow('unknown kubo type');
    });

    test('kubo without main div — search result variant', async () => {
      const a1name = mkNode('a', null, [mkText('TestShow-動作片')], { href: '/link' });
      const tr = mkNode('tr', null, [
        mkNode('td', null, [
          mkNode('div', null, [
            mkNode('a', null, [mkNode('img', null, [], { src: 'http://thumb.jpg' })], { href: '/vod-42.html' }),
          ]),
        ]),
        mkNode('td', null, [
          mkNode('h3', null, [a1name]),
          mkNode('div', null, [
            mkNode('div', 'kv', [
              mkNode('span', null, [
                mkText('月熱度:500'),
                mkNode('cite', null, [mkText('2020年05月01日')]),
              ]),
            ]),
          ]),
          mkNode('div', 'osl', [mkText('别名:AltName')]),
        ]),
      ]);
      const g = mkNode('li', 'g', [mkNode('table', null, [tr])]);
      const body = mkNode('body', null, [
        mkNode('div', null, [
          mkNode('div', 'wrapper_wrapper', [
            mkNode('div', 'container', [
              mkNode('div', 'content_left', [
                mkNode('div', 'ires', [
                  mkNode('ol', null, [g]),
                ]),
              ]),
      ...
      mockParseDOM.mockReturnValue([mkNode('html', null, [body])]);
      mockApi.mockResolvedValue('');
      const result = await ExternalTool.getSingleList('kubo', 'http://kubo.tv/search');
      expect(result).toHaveLength(1);
      expect(result[0].id).toBe('42');
    });

    test('kubo type_id 3 (animation tags)', async () => {
      const img = mkNode('img', null, [], { alt: 'Anime', 'data-original': 'http://x.jpg' });
      const li = mkNode('li', null, [mkNode('a', null, [img], { href: '/vod-1.html' })]);
      const body = mkNode('body', null, [
        mkNode('div', 'main', [
          mkNode('div', 'list', [mkNode('div', 'listlf', [mkNode('ul', null, [li])])]),
        ]),
      ]);
      mockParseDOM.mockReturnValue([mkNode('html', null, [body])]);
      mockApi.mockResolvedValue('');
      const result = await ExternalTool.getSingleList('kubo', 'http://kubo.tv/vod-search-id-3-page-1.html');
      expect(result[0].tags).toContain('動畫');
    });

    test('kubo type_id 2 (tv show)', async () => {
      const img = mkNode('img', null, [], { alt: 'TV', 'data-original': 'http://x.jpg' });
      const li = mkNode('li', null, [mkNode('a', null, [img], { href: '/vod-1.html' })]);
      const body = mkNode('body', null, [
        mkNode('div', 'main', [
          mkNode('div', 'list', [mkNode('div', 'listlf', [mkNode('ul', null, [li])])]),
      ...
      mockParseDOM.mockReturnValue([mkNode('html', null, [body])]);
      mockApi.mockResolvedValue('');
      const result = await ExternalTool.getSingleList('kubo', 'http://kubo.tv/vod-search-id-2-page-1.html');
      expect(result[0].tags).toContain('電視劇');
    });

    test('kubo type_id 41 (variety show)', async () => {
      const img = mkNode('img', null, [], { alt: 'Var', 'data-original': 'http://x.jpg' });
      const li = mkNode('li', null, [mkNode('a', null, [img], { href: '/vod-1.html' })]);
      const body = mkNode('body', null, [
        mkNode('div', 'main', [
          mkNode('div', 'list', [mkNode('div', 'listlf', [mkNode('ul', null, [li])])]),
      ...
      mockParseDOM.mockReturnValue([mkNode('html', null, [body])]);
      mockApi.mockResolvedValue('');
      const result = await ExternalTool.getSingleList('kubo', 'http://kubo.tv/vod-search-id-41-page-1.html');
      expect(result[0].tags).toContain('綜藝節目');
    });
  });
...
describe('youtubePlaylist', () => {
  test('returns first item when index === 1', async () => {
    mockGoogleApi.mockResolvedValue([
      [{ id: 'v1', index: 1 }, { id: 'v2', index: 2 }],
      2, 'nextToken', 'prevToken',
    ]);
    const result = await ExternalTool.youtubePlaylist('PLxxx', 1, null, false);
    expect(result[0].id).toBe('v1');
    expect(result[1]).toBe(false);
    expect(result[2]).toBe(2);
    expect(result[7]).toBe(false); // is_new = false
  });

  test('returns matching item when index matches id', async () => {
    mockGoogleApi.mockResolvedValue([
      [{ id: 'v1', index: 1 }, { id: 'v2', index: 2 }],
      2, null, null,
    ]);
    const result = await ExternalTool.youtubePlaylist('PLxxx', 'v2', null, false);
    expect(result[0].id).toBe('v2');
    expect(result[7]).toBe(false); // is_new = false
  });

  test('returns first item with is_new=true when no match', async () => {
    mockGoogleApi.mockResolvedValue([
      [{ id: 'v1', index: 1 }], 1, null, null,
    ]);
    const result = await ExternalTool.youtubePlaylist('PLxxx', 'nonexistent', null, false);
    expect(result[0].id).toBe('v1');
    expect(result[7]).toBe(true); // is_new = true
  });

  test('back=true returns last item', async () => {
    mockGoogleApi.mockResolvedValue([
      [{ id: 'v1' }, { id: 'v2' }], 2, null, null,
    ]);
    const result = await ExternalTool.youtubePlaylist('PLxxx', 'nomatch', null, true);
    expect(result[0].id).toBe('v2');
  });

  test('empty playlist → handleError', async () => {
    mockGoogleApi.mockResolvedValue([[], 0, null, null]);
    await expect(ExternalTool.youtubePlaylist('PLxxx', 1))
      .rejects.toThrow('playlist is empty');
  });

  test('with pageToken', async () => {
    mockGoogleApi.mockResolvedValue([[{ id: 'v1' }], 1, null, null]);
    const result = await ExternalTool.youtubePlaylist('PLxxx', 1, 'token123');
    expect(mockGoogleApi).toHaveBeenCalledWith('y playItem', { id: 'PLxxx', pageToken: 'token123' });
  });
});
...
describe('getSingleId', () => {
  ...
  test('non-numeric index for non-youtube → error', async () => {
    await expect(ExternalTool.getSingleId('eztv', 'http://x', 'abc'))
      .rejects.toThrow('index invalid');
  });
  ...
  test('youtube — single video (no list=)', async () => {
    const result = await ExternalTool.getSingleId('youtube', 'http://youtube.com/watch?v=ABCDE', 1);
    expect(result[0].id).toBe('you_ABCDE');
    expect(result[2]).toBe(1);
  });

  test('youtube — playlist (list=)', async () => {
    mockGoogleApi.mockResolvedValue([
      [{ id: 'v1', index: 1 }], 1, null, null,
    ]);
    const result = await ExternalTool.getSingleId('youtube', 'http://youtube.com/watch?v=X&list=PLabc', 1);
    expect(result[0].id).toBe('v1');
  });

  test('youtube — string index accepted', async () => {
    const result = await ExternalTool.getSingleId('youtube', 'http://youtube.com/watch?v=X', 'v1');
    // string index for youtube is accepted (no error)
    expect(result[0].id).toBe('you_X');
  });

  test('sub_index calculation from decimal index', async () => {
    // index=1.5 → sub_index = round(1.5*1000)%1000 = 1500%1000 = 500
    const result = await ExternalTool.getSingleId('youtube', 'http://youtube.com/watch?v=X', 1.5);
    expect(result[0].id).toBe('you_X');
  });

  test('sub_index=0 becomes 1', async () => {
    // index=1.0 → sub_index = 1000%1000 = 0 → becomes 1
    const result = await ExternalTool.getSingleId('youtube', 'http://youtube.com/watch?v=X', '1.0');
    expect(result[0].id).toBe('you_X');
  });
  ...
  // ----- lovetv -----
  describe('lovetv', () => {
    test('lovetv — invalid url → error', async () => {
      await expect(ExternalTool.getSingleId('lovetv', 'invalid-url', 1))
        .rejects.toThrow('invaild url');
    });

    test('lovetv — redis cached, returns ope_ id', async () => {
      mockRedis.mockResolvedValue({
        raw_list: JSON.stringify([{ name: 'Episode 1', url: '/ep1' }]),
        is_end: 'false',
        etime: '999999',
      });
      const result = await ExternalTool.getSingleId('lovetv', 'http://lovetv.com/show', 1);
      expect(result[0].id).toMatch(/^ope_/);
      expect(result[0].title).toBe('Episode 1');
    });

    test('lovetv — redis miss, fetches list', async () => {
      mockRedis.mockResolvedValue(null);
      // Build a DOM with table structure
      const a = mkNode('a', null, [mkText('Episode 1')], { href: '/ep1' });
      const h3 = mkNode('h3', null, [a]);
      const td = mkNode('td', null, [h3]);
      const tr = mkNode('tr', null, [td]);
      const table = mkNode('table', null, [tr]);
      const postBody = mkNode('div', 'post-body entry-content', [table]);
      const div0 = mkNode('div', null, [postBody]);
      const postOuter = mkNode('div', 'post-outer', [div0]);
      const datePosts = mkNode('div', 'date-posts', [postOuter]);
      const dateOuter = mkNode('div', 'date-outer', [datePosts]);
      const blogPosts = mkNode('div', 'blog-posts hfeed', [dateOuter]);
      const blog1 = mkNode('div', 'Blog1', [blogPosts]);
      const mainDiv = mkNode('div', 'main', [blog1]);
      const content = mkNode('div', 'content', [
        mkNode('div', 'content-outer', [
          mkNode('div', 'fauxborder-left content-fauxborder-left', [
            mkNode('div', 'content-inner', [
              mkNode('div', 'main-outer', [
                mkNode('div', 'fauxborder-left main-fauxborder-left', [
                  mkNode('div', 'region-inner main-inner', [
                    mkNode('div', 'columns fauxcolumns', [
                      mkNode('div', 'columns-inner', [
                        mkNode('div', 'column-center-outer', [
                          mkNode('div', 'column-center-inner', [mainDiv]),
                        ]),
                      ]),
                    ]),
                  ]),
      ...
      mockParseDOM.mockReturnValue([html]);
      mockApi.mockResolvedValue('<html>');
      // Redis hmset for saveList
      mockRedis.mockImplementation((op, ...args) => {
        if (op === 'hgetall') return Promise.resolve(null);
...
describe('saveSingle', () => {
  ...
  test('kubo — extracts tags with KUBO_TYPE mapping', async () => {
    const img = mkNode('img', null, [], { alt: 'KuboTitle', src: 'http://thumb.jpg' });
    const vpic = mkNode('div', 'vpic', [img]);
    const pType = mkNode('p', null, [
      mkText('分類：'),
      mkNode('font', null, [mkText('FontTag')]),
      mkNode('a', null, [mkText('動作片')]),
    ]);
    const pAlias = mkNode('p', null, [mkText('別名:Alt1/Alt2/')]);
    const pCat = mkNode('p', null, [mkText('類型：'), mkNode('a', null, [mkText('TypeTag')])]);
    const vshow = mkNode('div', 'vshow', [pAlias, pCat, pType]);
    const vmain = mkNode('div', 'vmain', [vpic, vshow]);
    const datal = mkNode('div', 'datal', [vmain]);
    const main = mkNode('div', 'main', [datal]);
    const body = mkNode('body', null, [main]);
    mockParseDOM.mockReturnValue([mkNode('html', null, [body])]);
    mockApi.mockResolvedValue('');
    const result = await ExternalTool.saveSingle('kubo', '123');
    expect(result[0]).toBe('T(KuboTitle)');
    expect(result[3]).toBe('kubo');
  });
  ...
  test('eztv — extracts show info', async () => {
    const titleTag = mkNode('title', null, [mkText('TestShow Torrent Download')]);
    const head = mkNode('head', null, [titleTag]);
    const td0 = mkNode('td', null, [
      mkText('2020\nGenre:Drama,Action\nNetwork:HBO'),
      mkNode('a', null, [], { href: 'https://www.imdb.com/title/tt123/' }),
    ]);
    const tr1 = mkNode('tr', null, []);
    const tr2 = mkNode('tr', null, [mkNode('td', null, []), td0]);
    const table1 = mkNode('table', null, [tr1, tr2]);
    const table2 = mkNode('table', null, [tr1, tr2]);
    const div6 = mkNode('div', null, [table1, table2]);
    const divs = [
      mkNode('div', null, []), mkNode('div', null, []),
      mkNode('div', null, []), mkNode('div', null, []),
      mkNode('div', null, []), mkNode('div', null, []),
      div6,
    ];
    const headerHolder = mkNode('div', 'header_holder', divs);
    const body = mkNode('body', null, [headerHolder]);
    mockParseDOM.mockReturnValue([mkNode('html', null, [head, body])]);
    mockApi.mockResolvedValue('');
    const result = await ExternalTool.saveSingle('eztv', '123-test-show');
    expect(result[0]).toBe('TestShow');
    expect(result[3]).toBe('eztv');
  });
});
...
describe('bilibiliVideoUrl', () => {
  test('invalid url → error', async () => {
    await expect(bilibiliVideoUrl('http://bilibili.com/bad'))
      .rejects.toThrow('bilibili id invalid');
  });

  test('extracts embed url from api', async () => {
    mockApi.mockResolvedValue('{}');
    mockGetJson.mockReturnValue({
      list: [{ part: 'Part1' }, { part: 'Part2' }],
    });
    const result = await bilibiliVideoUrl('http://bilibili.com/av12345/');
    expect(result.title).toBe('Part1');
    expect(result.embed[0]).toContain('12345');
  });

  test('with index_N.html', async () => {
    mockApi.mockResolvedValue('{}');
    mockGetJson.mockReturnValue({
      list: [{ part: 'P1' }, { part: 'P2' }],
    });
    const result = await bilibiliVideoUrl('http://bilibili.com/12345/index_2.html');
    expect(result.title).toBe('P2');
  });

  test('json parse error → error', async () => {
    mockApi.mockResolvedValue('bad');
    mockGetJson.mockReturnValue(false);
    await expect(bilibiliVideoUrl('http://bilibili.com/av123/'))
      .rejects.toThrow('json parse error');
  });

  test('no list → error', async () => {
    mockApi.mockResolvedValue('{}');
    mockGetJson.mockReturnValue({});
    await expect(bilibiliVideoUrl('http://bilibili.com/av123/'))
      .rejects.toThrow('cannot get list');
  });
});
...
describe('kuboVideoUrl', () => {
  test('kdy — extracts youtube iframe', async () => {
    const iframe = mkNode('iframe', null, [], { src: 'http://www.youtube.com/embed/ABCDE' });
    const ytPlayer = mkNode('div', 'youtube-player', [iframe]);
    const container = mkNode('div', 'container', [ytPlayer]);
    const body = mkNode('body', null, [container]);
    mockParseDOM.mockReturnValue([mkNode('html', null, [body])]);
    mockApi.mockResolvedValue('<html>');
    // youtubeVideoUrl will be called recursively — mock youtubedl
    mockYoutubedl.mockResolvedValue({
      title: 'TestVid',
      formats: [{ format_note: 'other', ext: 'mp4', url: 'http://video.mp4' }],
    });
    const result = await kuboVideoUrl('kdy', 'http://58b.tv/page');
    expect(result.video).toHaveLength(1);
  });

  test('kdy — no iframe found → error', async () => {
    const ytPlayer = mkNode('div', 'youtube-player', []);
    const container = mkNode('div', 'container', [ytPlayer]);
    const body = mkNode('body', null, [container]);
    mockParseDOM.mockReturnValue([mkNode('html', null, [body])]);
    mockApi.mockResolvedValue('<html>');
    await expect(kuboVideoUrl('kdy', 'http://58b.tv/page'))
      .rejects.toThrow('cannot find mp4');
  });

  test('kdy — dailymotion iframe', async () => {
    const iframe = mkNode('iframe', null, [], { src: 'http://other.com/embed?url=DMVID' });
    const ytPlayer = mkNode('div', 'youtube-player', [iframe]);
    const container = mkNode('div', 'container', [ytPlayer]);
    const body = mkNode('body', null, [container]);
    mockParseDOM.mockReturnValue([mkNode('html', null, [body])]);
    mockApi.mockResolvedValue('<html>');
    mockYoutubedl.mockResolvedValue({
      title: 'DM', formats: [{ format_id: 'http-480', ext: 'mp4', url: 'https://dm.mp4' }],
    });
    const result = await kuboVideoUrl('kdy', 'http://58b.tv/page');
    expect(result.video).toHaveLength(1);
  });

  test('kdy — multiple iframes, subIndex capped', async () => {
    const iframe1 = mkNode('iframe', null, [], { src: 'http://www.youtube.com/embed/V1' });
    const iframe2 = mkNode('iframe', null, [], { src: 'http://www.youtube.com/embed/V2' });
    const ytPlayer = mkNode('div', 'youtube-player', [iframe1, iframe2]);
    const container = mkNode('div', 'container', [ytPlayer]);
    const body = mkNode('body', null, [container]);
    mockParseDOM.mockReturnValue([mkNode('html', null, [body])]);
    mockApi.mockResolvedValue('');
    mockYoutubedl.mockResolvedValue({
      title: 'V2', formats: [{ format_note: 'other', ext: 'mp4', url: 'http://v.mp4' }],
    });
    const result = await kuboVideoUrl('kdy', 'http://58b.tv/page', 5);
    // subIndex 5 > 2 iframes → capped to 2
    expect(result.sub).toBe(2);
  });

  test('kud — returns url directly', async () => {
    const result = await kuboVideoUrl('kud', 'http://some.url');
    expect(result.url).toEqual(['http://some.url']);
    expect(result.video).toEqual([]);
  });

  test('kyu — extracts mp4 from API', async () => {
    mockApi.mockResolvedValue('{}');
    mockGetJson.mockReturnValue({
      code: '200',
      mp4: [{ url: 'http://v1.mp4' }, { url: 'http://v2.mp4' }],
    });
    const result = await kuboVideoUrl('kyu', 'http://58b.tv/video');
    expect(result.video).toEqual(['http://v1.mp4']);
    expect(result.sub).toBe(2);
  });

  test('kyu — code 404 → error', async () => {
    mockApi.mockResolvedValue('{}');
    mockGetJson.mockReturnValue({ code: '404' });
    await expect(kuboVideoUrl('kyu', 'http://58b.tv/video'))
      .rejects.toThrow('try later');
  });

  test('kyu — no mp4 → error', async () => {
    mockApi.mockResolvedValue('{}');
    mockGetJson.mockReturnValue({ code: '200' });
    await expect(kuboVideoUrl('kyu', 'http://58b.tv/video'))
      .rejects.toThrow('cannot find mp4');
  });

  test('kyu — mp4[subIndex-1] undefined → error', async () => {
    mockApi.mockResolvedValue('{}');
    mockGetJson.mockReturnValue({ code: '200', mp4: [] });
    await expect(kuboVideoUrl('kyu', 'http://58b.tv/video'))
      .rejects.toThrow('cannot find mp4');
  });

  test('kyu — json parse error → error', async () => {
    mockApi.mockResolvedValue('bad');
    mockGetJson.mockReturnValue(false);
    await expect(kuboVideoUrl('kyu', 'http://58b.tv/video'))
      .rejects.toThrow('json parse error');
  ...
  test('kyu — subIndex > mp4.length capped', async () => {
    mockApi.mockResolvedValue('{}');
    mockGetJson.mockReturnValue({ code: '200', mp4: [{ url: 'http://v.mp4' }] });
    const result = await kuboVideoUrl('kyu', 'http://58b.tv/video', 5);
    expect(result.video).toEqual(['http://v.mp4']);
  });

  test('unknown id — returns url array', async () => {
    const result = await kuboVideoUrl('other', 'http://some.url');
    expect(result.url).toEqual(['http://some.url']);
  });
});
...
describe('youtubeVideoUrl', () => {
  test('lin — returns iframe (no youtubedl call)', async () => {
    const result = await youtubeVideoUrl('lin', 'http://tv.line.me/v/12345');
    expect(result.iframe[0]).toContain('12345');
    expect(result.video).toEqual([]);
  });

  test('iqi — returns embed (no youtubedl call)', async () => {
    const result = await youtubeVideoUrl('iqi', 'http://iqiyi.com/abc-def-123-456.html');
    expect(result.embed[0]).toContain('abc');
    expect(result.video).toEqual([]);
  });

  test('you — extracts DASH audio + video', async () => {
    mockYoutubedl.mockResolvedValue({
      title: 'YouTubeVid',
      formats: [
        { format_note: 'DASH audio', filesize: 1000, url: 'http://audio1.mp3' },
        { format_note: 'DASH audio', filesize: 500, url: 'http://audio2.mp3' },
        { format_note: 'DASH video', ext: 'mp4', url: 'http://dv.mp4' },
        { format_note: '720p', ext: 'mp4', url: 'http://video.mp4' },
        { format_note: '360p', ext: 'webm', url: 'http://video.webm' },
      ],
    });
    const result = await youtubeVideoUrl('you', 'http://youtube.com/watch?v=X');
    expect(result.title).toBe('YouTubeVid');
    expect(result.audio).toBe('http://audio2.mp3');
    expect(result.video.length).toBeGreaterThanOrEqual(1);
  });

  test('you — DASH audio first is smallest already', async () => {
    mockYoutubedl.mockResolvedValue({
      title: 'Test',
      formats: [
        { format_note: 'DASH audio', filesize: 500, url: 'http://a1.mp3' },
        { format_note: 'DASH audio', filesize: 1000, url: 'http://a2.mp3' },
      ],
    });
    const result = await youtubeVideoUrl('you', 'http://youtube.com/watch?v=X');
    expect(result.audio).toBe('http://a1.mp3');
  });

  test('dym — extracts dailymotion videos', async () => {
    mockYoutubedl.mockResolvedValue({
      title: 'DMVid',
      formats: [
        { format_id: 'http-480', ext: 'mp4', url: 'https://dm.mp4' },
        { format_id: 'hls-auto', ext: 'mp4', url: 'http://hls.mp4' },
      ],
    });
    const result = await youtubeVideoUrl('dym', 'http://dailymotion.com/video/x123');
    expect(result.video.length).toBeGreaterThanOrEqual(1);
    expect(result.video[0]).toMatch(/^http:/);
  });

  test('yuk — adds iframe for flv type', async () => {
    mockYoutubedl.mockResolvedValue({
      title: 'YoukuVid',
      formats: [
        { ext: 'mp4', url: 'http://type=flv&v.mp4' },
      ],
    });
    const result = await youtubeVideoUrl('yuk', 'http://youku.com/id_ABC123=.html');
    expect(result.iframe).toBeDefined();
    expect(result.iframe[0]).toContain('ABC123=');
  });

  test('yuk — non-flv has empty iframe', async () => {
    mockYoutubedl.mockResolvedValue({
      title: 'YoukuVid',
      formats: [{ ext: 'mp4', url: 'http://normal.mp4' }],
    });
    const result = await youtubeVideoUrl('yuk', 'http://youku.com/id_ABC=.html');
    expect(result.iframe).toEqual([]);
  });

  test('generic id — array formats', async () => {
    mockYoutubedl.mockResolvedValue({
      title: 'GenVid',
      formats: [
        { ext: 'mp4', url: 'http://v1.mp4' },
        { ext: 'avi', url: 'http://v.avi' },
        { ext: 'webm', url: 'http://v.webm' },
      ],
    });
    const result = await youtubeVideoUrl('other', 'http://some.url');
    expect(result.video).toContain('http://v1.mp4');
    expect(result.video).toContain('http://v.webm');
    expect(result.video).not.toContain('http://v.avi');
  });

  test('generic id — non-array formats (single info)', async () => {
    mockYoutubedl.mockResolvedValue({
      title: 'Single',
      ext: 'mp4',
      url: 'http://single.mp4',
    ...
    const result = await youtubeVideoUrl('other', 'http://some.url');
    expect(result.video).toContain('http://single.mp4');
  });

  test('generic id — non-array with non-mp4 ext', async () => {
    mockYoutubedl.mockResolvedValue({
      title: 'X',
      ext: 'avi',
      url: 'http://x.avi',
    });
    const result = await youtubeVideoUrl('other', 'http://x');
    expect(result.video).toEqual([]);
  });

  test('lin inside youtubedl (fallback)', async () => {
    mockYoutubedl.mockResolvedValue({
      title: 'LinYT',
      formats: [{ ext: 'mp4', url: 'http://v.mp4' }],
    });
    const result = await youtubeVideoUrl('lin', 'http://tv.line.me/v/abc');
    // lin short-circuits before youtubedl
    expect(result.iframe[0]).toContain('abc');
...
// ===========================================================================
describe('edge cases', () => {
  test('getSingleList kubo — search variant with KUBO_TYPE matching', async () => {
    // name[2] matches KUBO_TYPE[1] → tv show
    const a1Inner = { type: 'text', data: 'ShowName-台灣劇' };
    const a1 = mkNode('a', null, [a1Inner], { href: '/link' });
    const h3 = mkNode('h3', null, [a1]);
    const span = mkNode('span', null, [
      mkText('月熱度:100'),
      mkNode('a', null, [mkText('tag1')]),
      mkNode('cite', null, [mkText('2020年05月01日')]),
    ]);
    const kv = mkNode('div', 'kv', [span]);
    const osl = mkNode('div', 'osl', [
      mkText('别名:AltNameX'),
      mkNode('a', null, [mkText('genre1')]),
    ]);
    const td1 = mkNode('td', null, [
      mkNode('div', null, [
        mkNode('a', null, [mkNode('img', null, [], { src: 'http://t.jpg' })], { href: '/vod-55.html' }),
      ]),
    ]);
    const td2 = mkNode('td', null, [h3, mkNode('div', null, [kv, osl])]);
    ...
    const body = mkNode('body', null, [
      mkNode('div', null, [
        mkNode('div', 'wrapper_wrapper', [
          mkNode('div', 'container', [
            mkNode('div', 'content_left', [
              mkNode('div', 'ires', [mkNode('ol', null, [g])]),
            ]),
          ]),
        ]),
      ]),
    ...
    const result = await ExternalTool.getSingleList('kubo', 'http://kubo.tv/search');
    expect(result).toHaveLength(1);
    expect(result[0].tags).toContain('tv show');
    expect(result[0].count).toBe(100);
    expect(result[0].date).toBe('2020-05-01');
  });

  test('getSingleList kubo search — findTag(s)[0] null in osl', async () => {
    const a1 = mkNode('a', null, [mkText('Title-X')], { href: '/link' });
    const span = mkNode('span', null, [mkText('other text'), mkNode('a', null, [])]);
    const kv = mkNode('div', 'kv', [span]);
    const osl = mkNode('div', 'osl', [mkNode('a', null, [])]);
    const td1 = mkNode('td', null, [mkNode('div', null, [mkNode('a', null, [mkNode('img', null, [], { src: 'http://t.jpg' })], { href: '/vod-1.html' })])]);
    const td2 = mkNode('td', null, [mkNode('h3', null, [a1]), mkNode('div', null, [kv, osl])]);
    const tr = mkNode('tr', null, [td1, td2]);
    const g = mkNode('li', 'g', [mkNode('table', null, [tr])]);
    const body = mkNode('body', null, [mkNode('div', null, [mkNode('div', 'wrapper_wrapper', [mkNode('div', 'container', [mkNode('div', 'content_left', [mkNode('div', 'ires', [mkNode('ol', null, [g])])])])])])]);
    mockParseDOM.mockReturnValue([mkNode('html', null, [body])]);
    mockApi.mockResolvedValue('');
    const result = await ExternalTool.getSingleList('kubo', 'http://kubo.tv/search');
    expect(result).toHaveLength(1);
  ...
  test('getSingleList kubo — p text children with child.data', async () => {
    // Tests the a1.children loop where c.data exists + findTag(c)[0] for <b>, findTag(c,'font')[0] for <em>
    // name must contain '-' for the regex match to succeed
    const a1child1 = { type: 'text', data: 'MyShow' };
    const a1child2 = mkNode('b', null, [mkText('-')]);
    const a1child3 = mkNode('em', null, [mkNode('font', null, [mkText('動作片')])]);
    const a1 = mkNode('a', null, [a1child1, a1child2, a1child3], { href: '/link' });
    const span = mkNode('span', null, [mkText('no match')]);
    const kv = mkNode('div', 'kv', [span]);
    const td1 = mkNode('td', null, [mkNode('div', null, [mkNode('a', null, [mkNode('img', null, [], { src: '' })], { href: '/vod-1.html' })])]);
    const td2 = mkNode('td', null, [mkNode('h3', null, [a1]), mkNode('div', null, [kv])]);
    const tr = mkNode('tr', null, [td1, td2]);
    const g = mkNode('li', 'g', [mkNode('table', null, [tr])]);
    const body = mkNode('body', null, [mkNode('div', null, [mkNode('div', 'wrapper_wrapper', [mkNode('div', 'container', [mkNode('div', 'content_left', [mkNode('div', 'ires', [mkNode('ol', null, [g])])])])])])]);
    mockParseDOM.mockReturnValue([mkNode('html', null, [body])]);
  ...
  test('getSingleId eztv — redis cached with magnet', async () => {
    const magnet = 'magnet:?xt=urn:btih:EZTV_HASH&extra';
    mockRedis.mockResolvedValue({
      raw_list: JSON.stringify([
        [{ magnet, name: 'Ep1', season: '001001', size: 300 }],
      ]),
      is_end: 'true',
      etime: '999999',
    });
    mockIsValidString.mockReturnValue(magnet);
    mockMongo.mockResolvedValue([]);
    const result = await ExternalTool.getSingleId('eztv', 'http://eztv.re/shows/123/', 1);
    expect(result[0].is_magnet).toBe(true);
  });

  test('getSingleId bilibili — redis cached', async () => {
    mockRedis.mockResolvedValue({
      raw_list: JSON.stringify([{ id: 'bil_av1', name: 'ep1' }]),
      is_end: 'false',
      etime: '999999',
    });
    const result = await ExternalTool.getSingleId('bilibili', 'http://bilibili.com/av123', 1);
    expect(result[0].id).toBe('bil_av1');
  });

  test('getSingleId kubo — redis cached, kdy prefix', async () => {
    mockRedis.mockResolvedValue({
      raw_list: JSON.stringify([{ id: 'kdy_abc', name: 'ep1' }]),
      is_end: 'false',
      etime: '999999',
    });
    const result = await ExternalTool.getSingleId('kubo', 'http://99kubo.tv/vod/1', 1);
    expect(result[0].id).toMatch(/^kdy_abc_/);
  });

  test('getSingleId kubo — redis cached, non-kdy', async () => {
    mockRedis.mockResolvedValue({
      raw_list: JSON.stringify([{ id: 'kur_abc', name: 'ep1' }]),
      is_end: 'false',
      etime: '999999',
    });
    const result = await ExternalTool.getSingleId('kubo', 'http://99kubo.tv/vod/1', 1);
    expect(result[0].id).toBe('kur_abc');
  });

  // --- Kubo movie genre switch cases (KUBO_TYPE[0] indices 1-7) ---
  // Helper to build kubo search DOM for a given genre
  const mkKuboSearchDOM = (genreName) => {
    const a1Inner = { type: 'text', data: `ShowName-${genreName}` };
    const a1 = mkNode('a', null, [a1Inner], { href: '/link' });
    const h3 = mkNode('h3', null, [a1]);
    const span = mkNode('span', null, [mkText('月熱度:50')]);
    const kv = mkNode('div', 'kv', [span]);
    const td1 = mkNode('td', null, [
      mkNode('div', null, [
        mkNode('a', null, [mkNode('img', null, [], { src: 'http://t.jpg' })], { href: '/vod-1.html' }),
      ]),
    ]);
    const td2 = mkNode('td', null, [h3, mkNode('div', null, [kv])]);
    const tr = mkNode('tr', null, [td1, td2]);
    const g = mkNode('li', 'g', [mkNode('table', null, [tr])]);
    return mkNode('body', null, [
      mkNode('div', null, [
        mkNode('div', 'wrapper_wrapper', [
          mkNode('div', 'container', [
            mkNode('div', 'content_left', [
  ...
  test('kubo search — KUBO_TYPE[0] comedy (index 1)', async () => {
    const body = mkKuboSearchDOM('喜劇片');
    mockParseDOM.mockReturnValue([mkNode('html', null, [body])]);
    mockApi.mockResolvedValue('');
    const result = await ExternalTool.getSingleList('kubo', 'http://kubo.tv/search');
    expect(result[0].tags).toContain('comedy');
    expect(result[0].tags).toContain('movie');
  });

  test('kubo search — KUBO_TYPE[0] romance (index 2)', async () => {
    const body = mkKuboSearchDOM('愛情片');
    mockParseDOM.mockReturnValue([mkNode('html', null, [body])]);
    mockApi.mockResolvedValue('');
    const result = await ExternalTool.getSingleList('kubo', 'http://kubo.tv/search');
    expect(result[0].tags).toContain('romance');
  });

  test('kubo search — KUBO_TYPE[0] sci-fi (index 3)', async () => {
    const body = mkKuboSearchDOM('科幻片');
    mockParseDOM.mockReturnValue([mkNode('html', null, [body])]);
    mockApi.mockResolvedValue('');
    const result = await ExternalTool.getSingleList('kubo', 'http://kubo.tv/search');
  ...
  test('kubo search — KUBO_TYPE[0] horror (index 4)', async () => {
    const body = mkKuboSearchDOM('恐怖片');
    mockParseDOM.mockReturnValue([mkNode('html', null, [body])]);
    mockApi.mockResolvedValue('');
    const result = await ExternalTool.getSingleList('kubo', 'http://kubo.tv/search');
    expect(result[0].tags).toContain('horror');
  });

  test('kubo search — KUBO_TYPE[0] drama (index 5)', async () => {
    const body = mkKuboSearchDOM('劇情片');
    mockParseDOM.mockReturnValue([mkNode('html', null, [body])]);
    mockApi.mockResolvedValue('');
    const result = await ExternalTool.getSingleList('kubo', 'http://kubo.tv/search');
    expect(result[0].tags).toContain('drama');
  });

  test('kubo search — KUBO_TYPE[0] war (index 6)', async () => {
    const body = mkKuboSearchDOM('戰爭片');
    mockParseDOM.mockReturnValue([mkNode('html', null, [body])]);
    mockApi.mockResolvedValue('');
    const result = await ExternalTool.getSingleList('kubo', 'http://kubo.tv/search');
    expect(result[0].tags).toContain('war');
  });

  test('kubo search — KUBO_TYPE[0] animation (index 7)', async () => {
    const body = mkKuboSearchDOM('動畫片');
    mockParseDOM.mockReturnValue([mkNode('html', null, [body])]);
    mockApi.mockResolvedValue('');
    const result = await ExternalTool.getSingleList('kubo', 'http://kubo.tv/search');
    expect(result[0].tags).toContain('animation');
  ...
  test('kubo search — KUBO_TYPE[2] variety', async () => {
    const body = mkKuboSearchDOM('綜藝');
    mockParseDOM.mockReturnValue([mkNode('html', null, [body])]);
    mockApi.mockResolvedValue('');
    const result = await ExternalTool.getSingleList('kubo', 'http://kubo.tv/search');
    expect(result[0].tags).toContain('綜藝節目');
  ...
  test('kubo search — KUBO_TYPE[3] animation type', async () => {
    const body = mkKuboSearchDOM('動漫');
    mockParseDOM.mockReturnValue([mkNode('html', null, [body])]);
    mockApi.mockResolvedValue('');
    const result = await ExternalTool.getSingleList('kubo', 'http://kubo.tv/search');
    expect(result[0].tags).toContain('animation');
  ...
  // --- getSingleId youtube — v= url (no playlist) returns directly ---
  test('getSingleId youtube — v= url returns fixed object', async () => {
    mockRedis.mockResolvedValue(null);
    const result = await ExternalTool.getSingleId('youtube', 'http://youtube.com/watch?v=ABC123', 1);
    expect(result[0].id).toBe('you_ABC123');
    expect(result[1]).toBe(false);
    expect(result[2]).toBe(1);
  });
  ...
  // --- getSingleId lovetv — redis cached path ---
  test('getSingleId lovetv — redis cached, returns episode', async () => {
    mockRedis.mockResolvedValue({
      raw_list: JSON.stringify([{ name: 'Episode 1', url: 'http://lovetv.tv/watch/ep1' }]),
      is_end: 'false',
      etime: '999999',
    });
    const result = await ExternalTool.getSingleId('lovetv', 'http://lovetv.tv/show/123', 1);
    expect(result[0].title).toBe('Episode 1');
    expect(result[0].id).toMatch(/^ope_/);
  });
  ...
  // --- getSingleId eztv non-cached (search URL) ---
  test('getSingleId eztv — non-cached search URL', async () => {
    mockRedis.mockResolvedValue(null);
    const magnetA = mkNode('a', 'magnet', [], {
      href: 'magnet:?xt=urn:btih:ABCDEF',
      title: 'Show.Name S01E01 720p',
    });
    const nameA = mkNode('a', null, [mkText('Show.Name.S01E01')]);
    const td0 = mkNode('td', 'forum_thread_post', []);
    const td1 = mkNode('td', 'forum_thread_post', [nameA]);
    const td2 = mkNode('td', 'forum_thread_post', [magnetA]);
    const td3 = mkNode('td', 'forum_thread_post', [mkText('500.0 M')]);
    const tr = mkNode('tr', 'forum_header_border', [td0, td1, td2, td3]);
    const table = mkNode('table', 'forum_header_border', [tr]);
    const skip1 = mkNode('table', 'forum_header_border', []);
    const skip2 = mkNode('table', 'forum_header_border', []);
    const headerHolder = mkNode('div', 'header_holder', [skip1, skip2, table]);
    const body = mkNode('body', null, [headerHolder]);
    mockParseDOM.mockReturnValue([mkNode('html', null, [body])]);
    mockApi.mockResolvedValue('<html>');
    mockIsValidString.mockReturnValue('magnet:?xt=urn:btih:ABCDEF');
    mockMongo.mockResolvedValue([]);
    const result = await ExternalTool.getSingleId('eztv', 'https://eztv.re/search/test', 1);
    expect(result[0].is_magnet).toBe(true);
    expect(result[0].magnet).toBe('magnet:?xt=urn:btih:ABCDEF');
  });
  ...
  // getSingleId bilibili — av URL, no select, single item (L2410-2418)
  test('getSingleId bilibili — av URL, no select → single item', async () => {
    mockRedis.mockResolvedValue(null);
    const body = mkNode('body', null, [mkNode('div', 'b-page-body', [mkNode('div', 'player-wrapper', [mkNode('div', 'main-inner', [mkNode('div', 'v-plist', [mkNode('div', 'plist', [])])])])])]);
    mockParseDOM.mockReturnValue([mkNode('html', null, [body])]);
    mockApi.mockResolvedValue('<html>');
    mockMongo.mockResolvedValue([]);
    ...
    const result = await ExternalTool.getSingleId('bilibili', 'http://www.bilibili.com/video/av12345', 1);
    expect(result[0].id).toMatch(/^bil_av12345$/);
  });

  // getSingleId bilibili — non-av URL, getBangumi (L2419-2425)
  test('getSingleId bilibili — non-av URL, getBangumi with seasons', async () => {
    mockRedis.mockImplementation((cmd, key) => {
      if (cmd === 'hgetall') return Promise.resolve(null);
      return Promise.resolve();
    });
    ...
    mockApi.mockImplementation(() => {
      apiCallCount++;
      return Promise.resolve('seasonListCallback({"result":{"episodes":[{"av_id":100,"page":1,"index_title":"Ep1"}],"seasons":[]}});');
    });
    mockGetJson.mockImplementation(s => {
      try { return JSON.parse(s); } catch { return false; }
    });
    mockMongo.mockResolvedValue([]);
    const result = await ExternalTool.getSingleId('bilibili', 'http://bangumi.bilibili.com/anime/12345', 1);
    expect(result[0].id).toBe('bil_av100_1');
    expect(result[0].title).toBe('Ep1');
  });
  ...
  // getSingleId eztv — direct URL (non-search), < 100 tr (L2255-2268)
  test('getSingleId eztv — direct URL with < 100 tr', async () => {
    mockRedis.mockImplementation((cmd, key) => {
      if (cmd === 'hgetall') return Promise.resolve(null);
      return Promise.resolve();
    });
    const magnetA = mkNode('a', 'magnet', [], { href: 'magnet:?xt=urn:btih:ABC', title: 'Show S01E02 720p' });
    const nameA = mkNode('a', null, [mkText('Show.S01E02')]);
    const td0 = mkNode('td', 'forum_thread_post', []);
    const td1 = mkNode('td', 'forum_thread_post', [nameA]);
    const td2 = mkNode('td', 'forum_thread_post', [magnetA]);
    const td3 = mkNode('td', 'forum_thread_post', [mkText('700.0 M')]);
    const tr = mkNode('tr', 'forum_header_border', [td0, td1, td2, td3]);
    // center > table.forum_header_noborder > tr.forum_header_border
    const tableInner = mkNode('table', 'forum_header_noborder', [tr]);
    // status table: table[0] > tr[4] > td[0] > b
    const statusB = mkNode('b', null, [mkText('Running')]);
    const statusTd = mkNode('td', null, [statusB]);
    const skipTr1 = mkNode('tr', null, []);
    const skipTr2 = mkNode('tr', null, []);
    const skipTr3 = mkNode('tr', null, []);
    const skipTr4 = mkNode('tr', null, []);
    const statusTr = mkNode('tr', null, [statusTd]);
    const statusTable = mkNode('table', null, [skipTr1, skipTr2, skipTr3, skipTr4, statusTr]);
    const center = mkNode('center', null, [tableInner, statusTable]);
    const innerTd = mkNode('td', null, [center]);
    const skipInnerTr = mkNode('tr', null, []);
    const innerTr = mkNode('tr', null, [innerTd]);
    const tableNormal = mkNode('table', 'forum_header_border_normal', [skipInnerTr, innerTr]);
    // div[6] so we need 7 divs, with the 7th being the one containing the table
    const skipDiv = mkNode('div', null, []);
    const headerHolder = mkNode('div', 'header_holder', [
      skipDiv, mkNode('div', null, []), mkNode('div', null, []),
      mkNode('div', null, []), mkNode('div', null, []), mkNode('div', null, []),
      mkNode('div', null, [tableNormal])
    ]);
    const body = mkNode('body', null, [headerHolder]);
    mockParseDOM.mockReturnValue([mkNode('html', null, [body])]);
    mockApi.mockResolvedValue('<html>');
    mockIsValidString.mockReturnValue('magnet:?xt=urn:btih:ABC');
    mockMongo.mockResolvedValue([]);
    const result = await ExternalTool.getSingleId('eztv', 'https://eztv.re/shows/123/', 1);
    expect(result[0].is_magnet).toBe(true);
  });

  // getSingleId eztv — duplicate season merge and size sorting (L2224-2242)
  test('getSingleId eztv — duplicate season entries sorted by size', async () => {
    mockRedis.mockImplementation((cmd, key) => {
      if (cmd === 'hgetall') return Promise.resolve(null);
      return Promise.resolve();
    });
    // two rows with same season but different sizes
    const magnetA1 = mkNode('a', 'magnet', [], { href: 'magnet:?xt=urn:btih:BIG', title: 'Show S01E01 1080p' });
    const nameA1 = mkNode('a', null, [mkText('Show.S01E01.1080p')]);
    const tr1 = mkNode('tr', 'forum_header_border', [
      mkNode('td', 'forum_thread_post', []),
      mkNode('td', 'forum_thread_post', [nameA1]),
      mkNode('td', 'forum_thread_post', [magnetA1]),
      mkNode('td', 'forum_thread_post', [mkText('2.5 G')]),
    ]);
    const magnetA2 = mkNode('a', 'magnet', [], { href: 'magnet:?xt=urn:btih:SMALL', title: 'Show S01E01 720p' });
    const nameA2 = mkNode('a', null, [mkText('Show.S01E01.720p')]);
    const tr2 = mkNode('tr', 'forum_header_border', [
      mkNode('td', 'forum_thread_post', []),
      mkNode('td', 'forum_thread_post', [nameA2]),
      mkNode('td', 'forum_thread_post', [magnetA2]),
      mkNode('td', 'forum_thread_post', [mkText('500.0 M')]),
    ]);
    const table = mkNode('table', 'forum_header_border', [tr1, tr2]);
    const skip1 = mkNode('table', 'forum_header_border', []);
    const skip2 = mkNode('table', 'forum_header_border', []);
    const headerHolder = mkNode('div', 'header_holder', [skip1, skip2, table]);
    const body = mkNode('body', null, [headerHolder]);
    mockParseDOM.mockReturnValue([mkNode('html', null, [body])]);
    mockApi.mockResolvedValue('<html>');
    mockIsValidString.mockReturnValue('magnet:?xt=urn:btih:BIG');
    mockMongo.mockResolvedValue([]);
    const result = await ExternalTool.getSingleId('eztv', 'https://eztv.re/search/test2', 1);
    expect(result[0].is_magnet).toBe(true);
  });

  // saveSingle eztv — non-cached (L2938-2956)
  test('saveSingle eztv — parses info and returns tags', async () => {
    const infoTd = mkNode('td', null, [
      mkText('2020'),
      mkText('Genre:Drama,Action'),
      mkText('Network:HBO'),
    ]);
    const imdbA = mkNode('a', null, [mkText('IMDB')], { href: 'https://www.imdb.com/title/tt1234567/' });
    infoTd.children.push(imdbA);
    const infoRow = mkNode('tr', null, [infoTd]);
    const skipRow = mkNode('tr', null, []);
    const table1 = mkNode('table', 'section_thread_post show_info_description', [skipRow, infoRow]);
    const centerEl = mkNode('center', null, [table1]);
    const centerTdWrap = mkNode('td', null, [centerEl]);
    const centerRow = mkNode('tr', null, [centerTdWrap]);
    const skipRowInner = mkNode('tr', null, []);
    const outerTable = mkNode('table', null, [skipRowInner, centerRow]);
    const titleText = mkNode('title', null, [mkText('TestShow Torrent Download EZTV')]);
    const head = mkNode('head', null, [titleText]);
    const div6 = mkNode('div', null, [outerTable]);
    const headerHolder = mkNode('div', 'header_holder', [
      mkNode('div', null, []), mkNode('div', null, []), mkNode('div', null, []),
      mkNode('div', null, []), mkNode('div', null, []), mkNode('div', null, []),
      div6,
    ...
    mockParseDOM.mockReturnValue([mkNode('html', null, [head, body])]);
    mockApi.mockResolvedValue('<html>');
    const result = await ExternalTool.saveSingle('eztv', '123');
    expect(result[0]).toBe('TestShow');
    expect(result[3]).toBe('eztv');
    expect([...result[1]]).toContain('2020');
    expect([...result[1]]).toContain('drama');
    expect([...result[1]]).toContain('hbo');
    expect([...result[1]]).toContain('tt1234567');
  });

  // saveSingle eztv with tables[1] path (when 2 tables exist)
  test('saveSingle eztv — tables[1] path with info', async () => {
    const infoTd = mkNode('td', null, [mkText('2021'), mkText('Genre:Comedy')]);
    const infoRow = mkNode('tr', null, [infoTd]);
    const skipRow = mkNode('tr', null, []);
    const table0 = mkNode('table', null, []);
    const table1 = mkNode('table', null, [skipRow, infoRow]);
    const div6 = mkNode('div', null, [table0, table1]);
    const titleText = mkNode('title', null, [mkText('AnotherShow Torrent Download EZTV')]);
    const head = mkNode('head', null, [titleText]);
    const headerHolder = mkNode('div', 'header_holder', [
      mkNode('div', null, []), mkNode('div', null, []), mkNode('div', null, []),
      mkNode('div', null, []), mkNode('div', null, []), mkNode('div', null, []),
    ...
    mockParseDOM.mockReturnValue([mkNode('html', null, [head, body])]);
    mockApi.mockResolvedValue('<html>');
    const result = await ExternalTool.saveSingle('eztv', '456');
    expect(result[0]).toBe('AnotherShow');
    expect([...result[1]]).toContain('2021');
    expect([...result[1]]).toContain('comedy');
  });
  ...
  // getSingleId eztv — "Ended" status detection (L2262-2264)
  test('getSingleId eztv — direct URL with Ended status', async () => {
    mockRedis.mockImplementation((cmd) => {
      if (cmd === 'hgetall') return Promise.resolve(null);
      return Promise.resolve();
    });
    const magnetA = mkNode('a', 'magnet', [], { href: 'magnet:?xt=urn:btih:XYZ', title: 'Show S01E01 720p' });
    const nameA = mkNode('a', null, [mkText('Show.S01E01')]);
    const dataTr = mkNode('tr', 'forum_header_border', [
      mkNode('td', 'forum_thread_post', []),
      mkNode('td', 'forum_thread_post', [nameA]),
      mkNode('td', 'forum_thread_post', [magnetA]),
      mkNode('td', 'forum_thread_post', [mkText('500.0 M')]),
    ]);
    // First table under center: needs at least 5 rows for tr[4] status check
    // tr[4] > td[0] > b with text "Ended"
    const statusB = mkNode('b', null, [mkText('Ended')]);
    const statusTd = mkNode('td', null, [statusB]);
    const statusTr = mkNode('tr', null, [statusTd]);
    const tableInner = mkNode('table', 'forum_header_noborder', [
      dataTr,
      mkNode('tr', null, []), mkNode('tr', null, []), mkNode('tr', null, []),
      statusTr
    ]);
    const center = mkNode('center', null, [tableInner]);
    const innerTd = mkNode('td', null, [center]);
    const innerTr = mkNode('tr', null, [innerTd]);
    const tableNormal = mkNode('table', 'forum_header_border_normal', [mkNode('tr', null, []), innerTr]);
    const headerHolder = mkNode('div', 'header_holder', [
      mkNode('div', null, []), mkNode('div', null, []), mkNode('div', null, []),
      mkNode('div', null, []), mkNode('div', null, []), mkNode('div', null, []),
      mkNode('div', null, [tableNormal])
    ...
    mockParseDOM.mockReturnValue([mkNode('html', null, [body])]);
    mockApi.mockResolvedValue('<html>');
    mockIsValidString.mockReturnValue('magnet:?xt=urn:btih:XYZ');
    mockMongo.mockResolvedValue([]);
    const result = await ExternalTool.getSingleId('eztv', 'https://eztv.re/shows/456/', 1);
    expect(result[0].is_magnet).toBe(true);
    expect(result[1]).toBe(true); // is_end = true because Ended found
  });

  // getSingleId eztv — >100 tr overflow path (L2270-2282)
  test('getSingleId eztv — direct URL with >100 tr falls back to search', async () => {
    mockRedis.mockImplementation((cmd) => {
      if (cmd === 'hgetall') return Promise.resolve(null);
      return Promise.resolve();
    });
    ...
    const trs = [];
    for (let i = 0; i < 105; i++) {
      const mag = mkNode('a', 'magnet', [], { href: `magnet:?xt=urn:btih:HASH${i}`, title: `Show S01E${String(i).padStart(2,'0')} 720p` });
      const name = mkNode('a', null, [mkText(`Show.S01E${String(i).padStart(2,'0')}`)]);
      trs.push(mkNode('tr', 'forum_header_border', [
        mkNode('td', 'forum_thread_post', []),
        mkNode('td', 'forum_thread_post', [name]),
        mkNode('td', 'forum_thread_post', [mag]),
        mkNode('td', 'forum_thread_post', [mkText('500.0 M')]),
      ]));
    }
    const tableInner = mkNode('table', 'forum_header_noborder', trs);
    const statusB = mkNode('b', null, [mkText('Running')]);
    const statusTd = mkNode('td', null, [statusB]);
    const statusTr = mkNode('tr', null, [statusTd]);
    const statusTable = mkNode('table', null, [
      mkNode('tr', null, []), mkNode('tr', null, []), mkNode('tr', null, []),
      mkNode('tr', null, []), statusTr
    ]);
    const center = mkNode('center', null, [tableInner, statusTable]);
    const innerTd = mkNode('td', null, [center]);
    const innerTr = mkNode('tr', null, [innerTd]);
    const tableNormal = mkNode('table', 'forum_header_border_normal', [mkNode('tr', null, []), innerTr]);
    const titleText = mkNode('title', null, [mkText('BigShow Torrent Download EZTV')]);
    const head = mkNode('head', null, [titleText]);
    const div6 = mkNode('div', null, [tableNormal]);
    const headerHolder = mkNode('div', 'header_holder', [
      mkNode('div', null, []), mkNode('div', null, []), mkNode('div', null, []),
      mkNode('div', null, []), mkNode('div', null, []), mkNode('div', null, []),
      div6
    ...
    // Search URL result (fewer rows)
    const searchTrs = [];
    for (let i = 0; i < 110; i++) {
      const mag2 = mkNode('a', 'magnet', [], { href: `magnet:?xt=urn:btih:SEARCH${i}`, title: `Show S01E${String(i).padStart(2,'0')} 720p` });
      const name2 = mkNode('a', null, [mkText(`Show.S01E${String(i).padStart(2,'0')}`)]);
      searchTrs.push(mkNode('tr', 'forum_header_border', [
        mkNode('td', 'forum_thread_post', []),
        mkNode('td', 'forum_thread_post', [name2]),
        mkNode('td', 'forum_thread_post', [mag2]),
        mkNode('td', 'forum_thread_post', [mkText('500.0 M')]),
      ]));
    }
    const searchTable = mkNode('table', 'forum_header_border', searchTrs);
    const searchSkip1 = mkNode('table', 'forum_header_border', []);
    const searchSkip2 = mkNode('table', 'forum_header_border', []);
    const searchHolder = mkNode('div', 'header_holder', [searchSkip1, searchSkip2, searchTable]);
    const searchBody = mkNode('body', null, [searchHolder]);
    ...
    mockParseDOM.mockImplementation(() => {
      if (callCount <= 1) return [mkNode('html', null, [head, body])];
      return [mkNode('html', null, [searchBody])];
    });
    mockIsValidString.mockReturnValue('magnet:?valid');
    mockMongo.mockResolvedValue([]);
    const result = await ExternalTool.getSingleId('eztv', 'https://eztv.re/shows/789/', 1);
    expect(result[0].is_magnet).toBe(true);
    expect(callCount).toBe(2); // second call is the search URL fallback
  });

  // getSingleId eztv — >100 tr, title does not contain " Torrent Download" → nameMatch is null → error
  test('getSingleId eztv — >100 tr with unrecognised title format → unknown name error', async () => {
    mockRedis.mockImplementation((cmd) => {
      if (cmd === 'hgetall') return Promise.resolve(null);
      return Promise.resolve();
    });
    // Build >100 tr rows (same DOM structure as existing >100 tr test, but bad title)
    const trs = [];
    for (let i = 0; i < 101; i++) {
      const mag = mkNode('a', 'magnet', [], { href: `magnet:?xt=urn:btih:X${i}`, title: `Show S01E${i}` });
      trs.push(mkNode('tr', 'forum_header_border', [
        mkNode('td', 'forum_thread_post', []),
        mkNode('td', 'forum_thread_post', [mkNode('a', null, [mkText(`Ep${i}`)])]),
        mkNode('td', 'forum_thread_post', [mag]),
        mkNode('td', 'forum_thread_post', [mkText('500 MB')]),
      ]));
    }
    const tableInner = mkNode('table', 'forum_header_noborder', trs);
    const statusTable = mkNode('table', null, [
      mkNode('tr', null, []), mkNode('tr', null, []), mkNode('tr', null, []),
      mkNode('tr', null, []), mkNode('tr', null, [mkNode('td', null, [mkNode('b', null, [mkText('Running')])])]),
    ]);
    const center = mkNode('center', null, [tableInner, statusTable]);
    const innerTd = mkNode('td', null, [center]);
    const innerTr = mkNode('tr', null, [innerTd]);
    const tableNormal = mkNode('table', 'forum_header_border_normal', [mkNode('tr', null, []), innerTr]);
    // Title does NOT contain " Torrent Download" → nameMatch = null → error
    const titleText = mkNode('title', null, [mkText('No Match Here')]);
    const head = mkNode('head', null, [titleText]);
    const div6 = mkNode('div', null, [tableNormal]);
    const headerHolder = mkNode('div', 'header_holder', [
      mkNode('div', null, []), mkNode('div', null, []), mkNode('div', null, []),
    ...
    mockParseDOM.mockReturnValue([mkNode('html', null, [head, body])]);
    mockApi.mockResolvedValue('<html>');
    await expect(ExternalTool.getSingleId('eztv', 'https://eztv.re/shows/999/', 1))
      .rejects.toThrow('unknown name!!!');
  });

  // getSingleId eztv — sendList recur_check finds existing magnet (L2310-2318)
  test('getSingleId eztv — sendList finds existing magnet via recur_check', async () => {
    const cachedList = [[
      { name: 'Show.S01E01.720p', magnet: 'magnet:?xt=urn:btih:SMALL', size: 500 },
      { name: 'Show.S01E01.1080p', magnet: 'magnet:?xt=urn:btih:BIG', size: 2500 },
    ]];
    mockRedis.mockImplementation((cmd) => {
      if (cmd === 'hgetall') return Promise.resolve({ raw_list: JSON.stringify(cachedList), is_end: 'false', etime: '100' });
      return Promise.resolve();
    });
    mockIsValidString.mockReturnValue('magnet:?xt=urn:btih:SMALL');
    mockMongo.mockResolvedValue([{ _id: 'existing-id-123' }]); // found existing
    const result = await ExternalTool.getSingleId('eztv', 'https://eztv.re/shows/123/', 1);
    expect(result[0].id).toBe('existing-id-123');
    expect(result[0].is_magnet).toBe(true);
  });

  // getSingleId eztv — sendList final_check when no recur_check match (L2301-2308)
  test('getSingleId eztv — sendList falls through to final_check', async () => {
    const cachedList = [[
      { name: 'Show.S01E01.720p', magnet: 'magnet:?xt=urn:btih:SMALL', size: 500 },
      { name: 'Show.S01E01.1080p', magnet: 'magnet:?xt=urn:btih:BIG', size: 2500 },
    ]];
    ...
    mockIsValidString.mockReturnValue('magnet:?xt=urn:btih:BIG');
    mockMongo.mockResolvedValue([]); // no match — recur_check falls through to final_check
    const result = await ExternalTool.getSingleId('eztv', 'https://eztv.re/shows/123/', 1);
    expect(result[0].magnet).toBe('magnet:?xt=urn:btih:BIG');
    expect(result[0].is_magnet).toBe(true);
  });

  // getSingleId eztv — sendList no choose — raw_list[index-1] is undefined → handleError (L2290-2292)
  test('getSingleId eztv — sendList index out of range', async () => {
    const cachedList = [[{ name: 'a', magnet: 'magnet:?xt=urn:btih:X', size: 100 }]];
    mockRedis.mockImplementation((cmd) => {
      if (cmd === 'hgetall') return Promise.resolve({ raw_list: JSON.stringify(cachedList), is_end: 'false', etime: '100' });
      return Promise.resolve();
    });
    // raw_list[98] is undefined → !raw → handleError('cannot find external index')
    await expect(ExternalTool.getSingleId('eztv', 'https://eztv.re/shows/123/', 99))
      .rejects.toThrow('cannot find external index');
  });

  // getSingleId eztv — final_check invalid magnet (L2302-2303)
  test('getSingleId eztv — final_check with invalid magnet', async () => {
    const cachedList = [[
      { name: 'Show', magnet: 'magnet:?xt=urn:btih:LAST', size: 100 },
    ]];
    mockRedis.mockImplementation((cmd) => {
      if (cmd === 'hgetall') return Promise.resolve({ raw_list: JSON.stringify(cachedList), is_end: 'false', etime: '100' });
      return Promise.resolve();
    });
    mockIsValidString.mockReturnValue(false); // invalid magnet
    mockMongo.mockResolvedValue([]); // no match in recur_check... wait, choose has 0 items after splice
    // choose = cachedList[0].slice() = [{...}], splice(0,1) = removes the single item → choose is empty
    // So choose.length === 0, goes directly to final_check
    // final_check: isValidString(chooseMag.magnet, 'url') → false → handleError
    await expect(ExternalTool.getSingleId('eztv', 'https://eztv.re/shows/123/', 1))
      .rejects.toThrow('magnet is not vaild');
  });

  // getSingleId eztv — duplicate size splice insert (L2236-2238)
  test('getSingleId eztv — search URL with size-sorted duplicates', async () => {
    mockRedis.mockImplementation((cmd) => {
      if (cmd === 'hgetall') return Promise.resolve(null);
      return Promise.resolve();
    });
    // Two entries with same season, second is larger → splice insert
    const mag1 = mkNode('a', 'magnet', [], { href: 'magnet:?xt=urn:btih:SMALL2', title: 'Show S01E01 720p' });
    const name1 = mkNode('a', null, [mkText('Show.S01E01.720p')]);
    const tr1 = mkNode('tr', 'forum_header_border', [
      mkNode('td', 'forum_thread_post', []),
      mkNode('td', 'forum_thread_post', [name1]),
      mkNode('td', 'forum_thread_post', [mag1]),
      mkNode('td', 'forum_thread_post', [mkText('500.0 M')]),
    ]);
    const mag2 = mkNode('a', 'magnet', [], { href: 'magnet:?xt=urn:btih:BIG2', title: 'Show S01E01 1080p' });
    const name2 = mkNode('a', null, [mkText('Show.S01E01.1080p')]);
    const tr2 = mkNode('tr', 'forum_header_border', [
      mkNode('td', 'forum_thread_post', []),
      mkNode('td', 'forum_thread_post', [name2]),
      mkNode('td', 'forum_thread_post', [mag2]),
    ...
    mockIsValidString.mockReturnValue('magnet:?valid');
    mockMongo.mockResolvedValue([]);
    const result = await ExternalTool.getSingleId('eztv', 'https://eztv.re/search/show-test', 1);
    expect(result[0].is_magnet).toBe(true);
  });

  // getSingleId bilibili — invalid id (L2391-2392)
  test('getSingleId bilibili — invalid id error', async () => {
    mockRedis.mockImplementation((cmd) => {
      if (cmd === 'hgetall') return Promise.resolve(null);
      return Promise.resolve();
    });
    await expect(ExternalTool.getSingleId('bilibili', 'http://bilibili.com/no-digits-here', 1))
      .rejects.toThrow('bilibili id invalid');
  });

  // getSingleId bilibili — getBangumi json error (L2396-2397)
  test('getSingleId bilibili — getBangumi json parse error', async () => {
    mockRedis.mockImplementation((cmd) => {
      if (cmd === 'hgetall') return Promise.resolve(null);
      return Promise.resolve();
    });
    mockApi.mockResolvedValue('seasonListCallback(bad json);');
    mockGetJson.mockReturnValue(false);
    await expect(ExternalTool.getSingleId('bilibili', 'http://bangumi.bilibili.com/anime/999', 1))
      .rejects.toThrow('json parse error');
  });

  // getSingleId bilibili — getBangumi no episodes (L2399-2400)
  test('getSingleId bilibili — getBangumi no episodes', async () => {
    mockRedis.mockImplementation((cmd) => {
      if (cmd === 'hgetall') return Promise.resolve(null);
      return Promise.resolve();
    });
    mockApi.mockResolvedValue('seasonListCallback({"result":{}});');
    mockGetJson.mockImplementation(s => { try { return JSON.parse(s); } catch { return false; } });
    await expect(ExternalTool.getSingleId('bilibili', 'http://bangumi.bilibili.com/anime/999', 1))
      .rejects.toThrow('cannot get episodes');
  });

  // getSingleId bilibili — sendList no choose (L2431-2432)
  test('getSingleId bilibili — sendList index out of range', async () => {
    mockRedis.mockImplementation((cmd) => {
      if (cmd === 'hgetall') return Promise.resolve({ raw_list: JSON.stringify([{id:'bil_1',name:'Ep1'}]), is_end: 'false', etime: '100' });
      return Promise.resolve();
    });
    await expect(ExternalTool.getSingleId('bilibili', 'http://bangumi.bilibili.com/anime/100', 99))
      .rejects.toThrow('cannot find external index');
  });

  // getSingleId bilibili — getBangumi with season recursion (L2420-2423)
  test('getSingleId bilibili — getBangumi with season recursion', async () => {
    mockRedis.mockImplementation((cmd) => {
      if (cmd === 'hgetall') return Promise.resolve(null);
      return Promise.resolve();
    });
    ...
    mockApi.mockImplementation(() => {
      apiCallCount++;
      if (apiCallCount === 1) {
        return Promise.resolve('seasonListCallback({"result":{"episodes":[{"av_id":100,"page":1,"index_title":"S1Ep1"}],"seasons":[{"season_id":200}]}});');
      }
      return Promise.resolve('seasonListCallback({"result":{"episodes":[{"av_id":200,"page":1,"index_title":"S2Ep1"}],"seasons":[]}});');
    });
    mockGetJson.mockImplementation(s => { try { return JSON.parse(s); } catch { return false; } });
    mockMongo.mockResolvedValue([]);
    const result = await ExternalTool.getSingleId('bilibili', 'http://bangumi.bilibili.com/anime/12345', 1);
    expect(result[0].id).toBe('bil_av100_1');
    expect(result[2]).toBe(2); // total 2 episodes
  });

  // getSingleId kubo — non-cached with 完結 and hideCont (L2451-2471)
  test('getSingleId kubo — non-cached with completeness and hideCont', async () => {
    mockRedis.mockImplementation((cmd) => {
      if (cmd === 'hgetall') return Promise.resolve(null);
      return Promise.resolve();
    });
    const liA = mkNode('a', null, [mkText('Episode 1')], { href: '/vod-play-1/' });
    const li = mkNode('li', null, [liA]);
    const innerDiv = mkNode('div', null, [li]);
    const innerUl = mkNode('ul', null, [innerDiv]);
    const vpl = mkNode('div', 'vpl', [innerUl]);
    const vmain2 = mkNode('div', 'vmain', [vpl]);
    const outerUl = mkNode('ul', null, [vmain2]);
    const hideCont = mkNode('div', 'hideCont', [outerUl]);
    const topRow = mkNode('div', 'topRow', [hideCont]);
    const pText = mkNode('p', null, [mkText('完結')]);
    const vshow = mkNode('div', 'vshow', [pText]);
    const vmain = mkNode('div', 'vmain', [vshow]);
    const datal = mkNode('div', 'datal', [vmain]);
    const mainDiv = mkNode('div', 'main', [datal, topRow]);
    const body = mkNode('body', null, [mainDiv]);
    mockParseDOM.mockReturnValue([mkNode('html', null, [body])]);
    mockApi.mockResolvedValue('<html>');
    mockMongo.mockResolvedValue([]);
    const result = await ExternalTool.getSingleId('kubo', 'http://kubo.example/show/1', 1);
    expect(result[1]).toBe(true); // is_end = true because of 完結
  });

  // getSingleId kubo — sendList no choose (L2541-2542)
  test('getSingleId kubo — sendList index out of range', async () => {
    mockRedis.mockImplementation((cmd) => {
      if (cmd === 'hgetall') return Promise.resolve({ raw_list: JSON.stringify([{id:'kur_1',name:'Ep'}]), is_end: 'false', etime: '100' });
      return Promise.resolve();
    });
    await expect(ExternalTool.getSingleId('kubo', 'http://kubo.example/show/1', 99))
      .rejects.toThrow('cannot find external index');
  });

  // getSingleId lovetv — no choose (L2120-2121)
  test('getSingleId lovetv — sendList index out of range', async () => {
    mockRedis.mockImplementation((cmd) => {
      if (cmd === 'hgetall') return Promise.resolve({ raw_list: JSON.stringify([{name:'Ep1',url:'/ep1'}]), is_end: 'false', etime: '100' });
      return Promise.resolve();
    });
    await expect(ExternalTool.getSingleId('lovetv', 'http://lovetv.example/show/1', 99))
      .rejects.toThrow('cannot find external index');
  });

  // saveSingle yify — non-cached page scrape (L2802-2828, saveSingle kubo switch)
  test('saveSingle kubo — parses type and genre info', async () => {
    const imgNode = mkNode('img', null, [], { alt: 'TestKubo', src: 'http://img.jpg' });
    const vpic = mkNode('div', 'vpic', [imgNode]);
    const pAlias = mkNode('p', null, [mkText('別名:AltName1/AltName2')]);
    const typeA = mkNode('a', null, [mkText('動作片')], { href: '/type/1' });
    const typeFont = mkNode('font', null, [mkText('SomeFont')]);
    const pType = mkNode('p', null, [mkText('分類：'), typeA, typeFont]);
    const genreA = mkNode('a', null, [mkText('ActionGenre')], { href: '/genre/1' });
    const pGenre = mkNode('p', null, [mkText('類型：'), genreA]);
    const pOther = mkNode('p', null, [mkText('Other text')]);
    const vshow = mkNode('div', 'vshow', [pAlias, pType, pGenre, pOther]);
    const vmain = mkNode('div', 'vmain', [vpic, vshow]);
    const datal = mkNode('div', 'datal', [vmain]);
    const mainDiv = mkNode('div', 'main', [datal]);
    const body = mkNode('body', null, [mainDiv]);
    mockParseDOM.mockReturnValue([mkNode('html', null, [body])]);
    mockApi.mockResolvedValue('<html>');
    const result = await ExternalTool.saveSingle('kubo', 'test-id');
    // OpenCC mock wraps with T(...)
    expect(result[0]).toBe('T(TestKubo)');
    expect(result[3]).toBe('kubo');
    // Tags go through OpenCC too
    expect([...result[1]]).toContain('T(AltName1)');
    expect([...result[1]]).toContain('T(AltName2)');
    expect([...result[1]]).toContain('T(movie)');
    expect([...result[1]]).toContain('T(電影)');
    expect([...result[1]]).toContain('T(action)');
    expect([...result[1]]).toContain('T(動作)');
    expect([...result[1]]).toContain('T(ActionGenre)');
  });

  // youtubeVideoUrl — generic id with array formats (L3169-3173)
  test('youtubeVideoUrl — generic format array', async () => {
    mockYoutubedl.mockResolvedValue({
      title: 'GenericVideo',
      formats: [
        { ext: 'mp4', url: 'http://video.com/a.mp4' },
        { ext: 'flv', url: 'http://video.com/b.flv' },
        { ext: 'webm', url: 'http://video.com/c.webm' },
      ],
    });
    const result = await youtubeVideoUrl('http://generic.com/video', 'gen');
    expect(result.video.length).toBe(2);
    expect(result.title).toBe('GenericVideo');
  });

  // youtubeVideoUrl — generic id with single format object (L3175-3178)
  test('youtubeVideoUrl — generic format single object', async () => {
    mockYoutubedl.mockResolvedValue({
      title: 'SingleVideo',
      ext: 'mp4',
      url: 'http://video.com/single.mp4',
    });
    const result = await youtubeVideoUrl('http://generic.com/video2', 'other');
    expect(result.video.length).toBe(1);
    expect(result.video[0]).toBe('http://video.com/single.mp4');
  });

  // --- saveSingle kubo — KUBO_TYPE switch cases L2802-2828 ---
  // Helper for kubo saveSingle DOM with configurable type
  const mkKuboSaveSingleDOM = (typeName) => {
    const imgNode = mkNode('img', null, [], { alt: 'KuboShow', src: 'http://img.jpg' });
    const vpic = mkNode('div', 'vpic', [imgNode]);
    const typeA = mkNode('a', null, [mkText(typeName)], { href: '/type' });
    const pType = mkNode('p', null, [mkText('分類：'), typeA]);
    const vshow = mkNode('div', 'vshow', [pType]);
    const vmain = mkNode('div', 'vmain', [vpic, vshow]);
    const datal = mkNode('div', 'datal', [vmain]);
    const mainDiv = mkNode('div', 'main', [datal]);
    return mkNode('body', null, [mainDiv]);
  };

  test('saveSingle kubo — KUBO_TYPE[0][1] comedy', async () => {
    mockParseDOM.mockReturnValue([mkNode('html', null, [mkKuboSaveSingleDOM('喜劇片')])]);
    mockApi.mockResolvedValue('');
    const result = await ExternalTool.saveSingle('kubo', 'id1');
    expect([...result[1]]).toContain('T(comedy)');
    expect([...result[1]]).toContain('T(喜劇)');
  });

  test('saveSingle kubo — KUBO_TYPE[0][2] romance', async () => {
    mockParseDOM.mockReturnValue([mkNode('html', null, [mkKuboSaveSingleDOM('愛情片')])]);
    mockApi.mockResolvedValue('');
    const result = await ExternalTool.saveSingle('kubo', 'id2');
    expect([...result[1]]).toContain('T(romance)');
  });

  test('saveSingle kubo — KUBO_TYPE[0][3] sci-fi', async () => {
    mockParseDOM.mockReturnValue([mkNode('html', null, [mkKuboSaveSingleDOM('科幻片')])]);
    mockApi.mockResolvedValue('');
    const result = await ExternalTool.saveSingle('kubo', 'id3');
    expect([...result[1]]).toContain('T(sci-fi)');
  });

  test('saveSingle kubo — KUBO_TYPE[0][4] horror', async () => {
    mockParseDOM.mockReturnValue([mkNode('html', null, [mkKuboSaveSingleDOM('恐怖片')])]);
    mockApi.mockResolvedValue('');
    const result = await ExternalTool.saveSingle('kubo', 'id4');
    expect([...result[1]]).toContain('T(horror)');
  });

  test('saveSingle kubo — KUBO_TYPE[0][5] drama', async () => {
    mockParseDOM.mockReturnValue([mkNode('html', null, [mkKuboSaveSingleDOM('劇情片')])]);
    mockApi.mockResolvedValue('');
    const result = await ExternalTool.saveSingle('kubo', 'id5');
    expect([...result[1]]).toContain('T(drama)');
  });

  test('saveSingle kubo — KUBO_TYPE[0][6] war', async () => {
    mockParseDOM.mockReturnValue([mkNode('html', null, [mkKuboSaveSingleDOM('戰爭片')])]);
    mockApi.mockResolvedValue('');
    const result = await ExternalTool.saveSingle('kubo', 'id6');
    expect([...result[1]]).toContain('T(war)');
  });

  test('saveSingle kubo — KUBO_TYPE[0][7] animation', async () => {
    mockParseDOM.mockReturnValue([mkNode('html', null, [mkKuboSaveSingleDOM('動畫片')])]);
    mockApi.mockResolvedValue('');
    const result = await ExternalTool.saveSingle('kubo', 'id7');
    expect([...result[1]]).toContain('T(animation)');
    expect([...result[1]]).toContain('T(動畫)');
  });

  test('saveSingle kubo — KUBO_TYPE[1] tv show', async () => {
    mockParseDOM.mockReturnValue([mkNode('html', null, [mkKuboSaveSingleDOM('台灣劇')])]);
    mockApi.mockResolvedValue('');
    const result = await ExternalTool.saveSingle('kubo', 'id8');
    expect([...result[1]]).toContain('T(tv show)');
    expect([...result[1]]).toContain('T(電視劇)');
  });

  test('saveSingle kubo — KUBO_TYPE[2] variety', async () => {
    mockParseDOM.mockReturnValue([mkNode('html', null, [mkKuboSaveSingleDOM('綜藝')])]);
    mockApi.mockResolvedValue('');
    const result = await ExternalTool.saveSingle('kubo', 'id9');
    expect([...result[1]]).toContain('T(tv show)');
    expect([...result[1]]).toContain('T(綜藝節目)');
  });

  test('saveSingle kubo — KUBO_TYPE[3] anime', async () => {
    mockParseDOM.mockReturnValue([mkNode('html', null, [mkKuboSaveSingleDOM('動漫')])]);
    mockApi.mockResolvedValue('');
    const result = await ExternalTool.saveSingle('kubo', 'id10');
    expect([...result[1]]).toContain('T(animation)');
    expect([...result[1]]).toContain('T(動畫)');
  });
  ...
  // getSingleId eztv — >100 tr with empty name (L2272-2273 is dead code — findTag trims text, empty match is impossible)

  // getSingleId lovetv — non-cached, non-table (outer) path (L2050-2064)
  test('getSingleId lovetv — non-cached outer path with 劇集列表', async () => {
    mockRedis.mockImplementation((cmd) => {
      if (cmd === 'hgetall') return Promise.resolve(null);
      return Promise.resolve();
    });
    // Build first page: no table → goes to outer iteration
    // outer = date-outer divs, each contains date-posts > post-outer > div > h3 > a
    const listA = mkNode('a', null, [mkText('劇集列表')], { href: 'http://lovetv.example/list' });
    const listH3 = mkNode('h3', null, [listA]);
    const listDiv = mkNode('div', null, [listH3]);
    const listPostOuter = mkNode('div', 'post-outer', [listDiv]);
    const listDatePosts = mkNode('div', 'date-posts', [listPostOuter]);
    const listDateOuter = mkNode('div', 'date-outer', [listDatePosts]);

    const blogPosts = mkNode('div', 'blog-posts hfeed', [listDateOuter]);
    const blog1 = mkNode('div', 'Blog1', [blogPosts]);
    const mainDiv = mkNode('div', 'main', [blog1]);
    const colInner = mkNode('div', 'column-center-inner', [mainDiv]);
    const colOuter = mkNode('div', 'column-center-outer', [colInner]);
    const columnsInner = mkNode('div', 'columns-inner', [colOuter]);
    const columns = mkNode('div', 'columns fauxcolumns', [columnsInner]);
    const regionInner = mkNode('div', 'region-inner main-inner', [columns]);
    const fauxLeft = mkNode('div', 'fauxborder-left main-fauxborder-left', [regionInner]);
    const mainOuter = mkNode('div', 'main-outer', [fauxLeft]);
    const contentInner = mkNode('div', 'content-inner', [mainOuter]);
    const fauxLeft2 = mkNode('div', 'fauxborder-left content-fauxborder-left', [contentInner]);
    const contentOuter = mkNode('div', 'content-outer', [fauxLeft2]);
    const content = mkNode('div', 'content', [contentOuter]);
    const body = mkNode('body', null, [content]);

    // Second call (from 劇集列表 link): page with table path
    const tableA = mkNode('a', null, [mkText('Ep 1')], { href: 'http://lovetv.example/watch/1' });
    const tableH3 = mkNode('h3', null, [tableA]);
    const tableTd = mkNode('td', null, [tableH3]);
    const tableTr = mkNode('tr', null, [tableTd]);
    const table = mkNode('table', null, [tableTr]);
    const postBody = mkNode('div', 'post-body entry-content', [table]);
    const postDiv = mkNode('div', null, [postBody]);
    const postOuter2 = mkNode('div', 'post-outer', [postDiv]);
    const datePosts2 = mkNode('div', 'date-posts', [postOuter2]);
    const dateOuter2 = mkNode('div', 'date-outer', [datePosts2]);
    const blogPosts2 = mkNode('div', 'blog-posts hfeed', [dateOuter2]);
    const blog12 = mkNode('div', 'Blog1', [blogPosts2]);
    const mainDiv2 = mkNode('div', 'main', [blog12]);
    const colInner2 = mkNode('div', 'column-center-inner', [mainDiv2]);
    const colOuter2 = mkNode('div', 'column-center-outer', [colInner2]);
    const columnsInner2 = mkNode('div', 'columns-inner', [colOuter2]);
    const columns2 = mkNode('div', 'columns fauxcolumns', [columnsInner2]);
    const regionInner2 = mkNode('div', 'region-inner main-inner', [columns2]);
    const fauxLeft3 = mkNode('div', 'fauxborder-left main-fauxborder-left', [regionInner2]);
    const mainOuter2 = mkNode('div', 'main-outer', [fauxLeft3]);
    const contentInner2 = mkNode('div', 'content-inner', [mainOuter2]);
    const fauxLeft4 = mkNode('div', 'fauxborder-left content-fauxborder-left', [contentInner2]);
    const contentOuter2 = mkNode('div', 'content-outer', [fauxLeft4]);
    const content2 = mkNode('div', 'content', [contentOuter2]);
    const body2 = mkNode('body', null, [content2]);
    ...
    mockParseDOM.mockImplementation(() => {
      if (apiCallCount <= 1) return [mkNode('html', null, [body])];
      return [mkNode('html', null, [body2])];
    });
    mockIsValidString.mockReturnValue('http://lovetv.example/watch/1');
    mockMongo.mockResolvedValue([]);
    const result = await ExternalTool.getSingleId('lovetv', 'http://lovetv.example/show/1', 1);
    expect(result[0].id).toContain('ope_');
  });

  // getSingleId lovetv — non-cached, body path (no content div) (L2067-2079)
  test('getSingleId lovetv — non-cached body path with table', async () => {
    mockRedis.mockImplementation((cmd) => {
      if (cmd === 'hgetall') return Promise.resolve(null);
      return Promise.resolve();
    });
    // Body path (no 'content' div): body > div.wrapper > div.main > div.container > div.content > div[2] > div.entry-content > table > tbody > tr
    const epA = mkNode('a', null, [mkText('Ep 1')], { href: 'http://lovetv.example/ep1' });
    const h3 = mkNode('h3', null, [epA]);
    const td = mkNode('td', null, [h3]);
    const tr = mkNode('tr', null, [td]);
    const tbody = mkNode('tbody', null, [tr]);
    const table = mkNode('table', null, [tbody]);
    const entryContent = mkNode('div', 'entry-content', [table]);
    const div2 = mkNode('div', null, [entryContent]);
    const contentDiv = mkNode('div', 'content', [mkNode('div', null, []), mkNode('div', null, []), div2]);
    const container = mkNode('div', 'container', [contentDiv]);
    const mainDiv = mkNode('div', 'main', [container]);
    const wrapper = mkNode('div', 'wrapper', [mainDiv]);
    const body = mkNode('body', null, [wrapper]);
    mockParseDOM.mockReturnValue([mkNode('html', null, [body])]);
    mockApi.mockResolvedValue('');
    mockIsValidString.mockReturnValue('http://lovetv.example/ep1');
    mockMongo.mockResolvedValue([]);
    const result = await ExternalTool.getSingleId('lovetv', 'http://lovetv.example/show/1', 1);
    expect(result[0].id).toContain('ope_');
  });

  // getSingleId lovetv — outer path with non-劇集列表/non-Synopsis episodes (L2058-2059)
  test('getSingleId lovetv — outer path with regular episode names', async () => {
    mockRedis.mockImplementation((cmd) => {
      if (cmd === 'hgetall') return Promise.resolve(null);
      return Promise.resolve();
    });
    // Outer element with a regular episode name (not 劇集列表, not Synopsis)
    const epA = mkNode('a', null, [mkText('第一集')], { href: 'http://lovetv.example/ep1' });
    const epH3 = mkNode('h3', null, [epA]);
    const epDiv = mkNode('div', null, [epH3]);
    const epPostOuter = mkNode('div', 'post-outer', [epDiv]);
    const epDatePosts = mkNode('div', 'date-posts', [epPostOuter]);
    const epDateOuter = mkNode('div', 'date-outer', [epDatePosts]);

    // Another with 大結局 in name for is_end check (L2083-2085)
    const endA = mkNode('a', null, [mkText('大結局')], { href: 'http://lovetv.example/finale' });
    const endH3 = mkNode('h3', null, [endA]);
    const endDiv = mkNode('div', null, [endH3]);
    const endPostOuter = mkNode('div', 'post-outer', [endDiv]);
    const endDatePosts = mkNode('div', 'date-posts', [endPostOuter]);
    const endDateOuter = mkNode('div', 'date-outer', [endDatePosts]);

    const blogPosts = mkNode('div', 'blog-posts hfeed', [epDateOuter, endDateOuter]);
    const blog1 = mkNode('div', 'Blog1', [blogPosts]);
    const mainDiv = mkNode('div', 'main', [blog1]);
    const colInner = mkNode('div', 'column-center-inner', [mainDiv]);
    const colOuter = mkNode('div', 'column-center-outer', [colInner]);
    ...
    mockMongo.mockResolvedValue([]);
    const result = await ExternalTool.getSingleId('lovetv', 'http://lovetv.example/show/1', 1);
    expect(result[1]).toBe(true); // is_end = true because 大結局 found
  });

  // getSingleId lovetv — body path (no content div) with 大結局 (L2067-2079, L2084-2085)
  test('getSingleId lovetv — body path with table and 大結局', async () => {
    mockRedis.mockImplementation((cmd) => {
      if (cmd === 'hgetall') return Promise.resolve(null);
      return Promise.resolve();
    });
    const ep1A = mkNode('a', null, [mkText('Ep 大結局')], { href: 'http://lovetv.example/ep1' });
    const h3_1 = mkNode('h3', null, [ep1A]);
    const td1 = mkNode('td', null, [h3_1]);
    const tr1 = mkNode('tr', null, [td1]);
    const ep2A = mkNode('a', null, [mkText('Ep 2')], { href: 'http://lovetv.example/ep2' });
    const h3_2 = mkNode('h3', null, [ep2A]);
    const td2 = mkNode('td', null, [h3_2]);
    const tr2 = mkNode('tr', null, [td2]);
    const tbody = mkNode('tbody', null, [tr1, tr2]);
    const table = mkNode('table', null, [tbody]);
    const entryContent = mkNode('div', 'entry-content', [table]);
    const div2 = mkNode('div', null, [entryContent]);
    const contentDiv = mkNode('div', 'content', [mkNode('div', null, []), mkNode('div', null, []), div2]);
    ...
    mockMongo.mockResolvedValue([]);
    const result = await ExternalTool.getSingleId('lovetv', 'http://lovetv.example/show/1', 1);
    expect(result[1]).toBe(true); // is_end because 大結局 found
    expect(result[0].id).toContain('ope_');
  });

  // getSingleId lovetv — Mongo fallback when list is empty (all Synopsis) and items.length < 1
  test('getSingleId lovetv — empty list, Mongo finds nothing → cannot find lovetv url', async () => {
    mockRedis.mockImplementation((cmd) => {
      if (cmd === 'hgetall') return Promise.resolve(null);
      return Promise.resolve();
    });
    // Build DOM with content path, table with one Synopsis row → list stays empty
    const a = mkNode('a', null, [mkText('Episode Synopsis')], { href: '/synopsis' });
    const h3 = mkNode('h3', null, [a]);
    const td = mkNode('td', null, [h3]);
    const tr = mkNode('tr', null, [td]);
    const table = mkNode('table', null, [tr]);
    ...
    mockParseDOM.mockReturnValue([mkNode('html', null, [body])]);
    mockApi.mockResolvedValue('<html>');
    mockMongo.mockResolvedValue([]); // Mongo returns no items
    await expect(ExternalTool.getSingleId('lovetv', 'http://lovetv.example/show/1', 1))
      .rejects.toThrow('cannot find lovetv url');
  });

  // getSingleId lovetv — Mongo fallback when list is empty and items found → cannot find lovetv
  test('getSingleId lovetv — empty list, Mongo finds stored item → cannot find lovetv', async () => {
    mockRedis.mockImplementation((cmd) => {
      if (cmd === 'hgetall') return Promise.resolve(null);
      return Promise.resolve();
    });
    ...
    mockParseDOM.mockReturnValue([mkNode('html', null, [body])]);
    mockApi.mockResolvedValue('<html>');
    // Mongo returns a stored item → previously tried nextLove+recur_loveList (dead code),
    // now correctly falls through to handleError('cannot find lovetv')
    mockMongo.mockResolvedValue([{ _id: 'stored-id', name: 'Some Show' }]);
    await expect(ExternalTool.getSingleId('lovetv', 'http://lovetv.example/show/1', 1))
      .rejects.toThrow('cannot find lovetv');
  });
});
