...
const getStockPrice = (type='twse', index, price_only = true) => {
    switch(type) {
        ...
        const real = () => Api('url', `https://tw.stock.yahoo.com/q/q?s=${index}`).then(raw_data => {
            const table1 = findTag(findTag(findTag(findTag(Htmlparser.parseDOM(raw_data), 'html')[0], 'body')[0], 'center')[0], 'table')[1];
            if (!table1) {
                return handleError(new HoError(`stock ${index} price get table1 fail`));
            }
            const table = findTag(findTag(findTag(table1, 'tr')[0], 'td')[0], 'table')[0];
            if (!table) {
    ...
    const recur_price = index => {
        if (index >= items.length) {
            ...
            return (items[index].index === 0 || !items[index].index) ? recur_price(index + 1) : getStockPrice(items[index].setype, items[index].index).then(price => {
                ...
                let suggestion = stockProcess(price, newArr, item.times, item.previous, item.orig, item.clear ? 0 : item.amount, item.count, 0, 0, item.wType, 0, fee);
...
export const stockProcess = (price, priceArray, priceTimes = 1, previous = {buy:[], sell:[]}, pOrig, pAmount, pCount, pPricecost, pPl, pType = 0, sType = 0, fee = TRADE_FEE, ttime = TRADE_TIME, tinterval = TRADE_INTERVAL, now = Math.round(new Date().getTime() / 1000)) => {
...
export const stockTest = (his_arr, loga, min, pType = 0, start = 0, reverse = false, len = 200, rinterval = RANGE_INTERVAL, fee = TRADE_FEE, ttime = TRADE_TIME, tinterval = TRADE_INTERVAL, resetWeb = 5, sType = 0) => {
    ...
    for (let i = startI; i > tlength; i--) {
        ...
        //if (is_start) {
            ...
            suggest = stockProcess(price, newArr, web.times, priviousTrade, maxAmount, amount, count, 0, 0, pType, sType, fee, ttime, tinterval, now - (i * tinterval));
