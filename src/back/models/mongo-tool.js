...
export const objectID = id => id === undefined ? new ObjectID() : new ObjectID(id)
...
export default function(functionName, name, ...args) {
    if (name in collections) {
        if (functionName === 'find') {
            ...
            return new Promise((resolve,reject) => collections[name][functionName].call(collections[name], ...args, (err,data) => err ? reject(err) : resolve(data)))
        }
    } else {
        return new Promise((resolve,reject) => db.collection(name, (err, collection) => err ? reject(err) : resolve(collection))).then(collection => {
            ...
            if (functionName === 'find') {
                ...
                return new Promise((resolve,reject) => collection[functionName].call(collection, ...args, (err,data) => err ? reject(err) : resolve(data)))
            }
        })
    }
}
