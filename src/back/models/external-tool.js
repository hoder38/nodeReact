    ...
    getSingleList: function(type, url, post=null) {
        ...
        switch (type) {
            ...
            return Api('url', 'https://www.dol.gov/newsroom/releases').then(raw_data => {
                ...
                console.log(docDate);
                let list = [];
                const divs = findTag(findTag(findTag(findTag(findTag(findTag(findTag(findTag(findTag(findTag(findTag(Htmlparser.parseDOM(raw_data), 'html')[0], 'body')[0], 'div', 'dialog-off-canvas-main-canvas')[0], 'div')[0], 'main', 'cd-main-content')[0], 'div', 'layout-content inner-content-page')[0], 'div')[0], 'div', 'block-opa-theme-content')[0], 'div', 'views-element-container')[0], 'div')[0], 'div');
                for (let i in divs) {
                ...
                for (let p of ps) {
                    ...
                    if (pc && pc.match(/本文及附表/)) {
                        ...
                        if (pcsp && pcsp.match(/本文及附表/)) {
                            ...
                            const url = a ? addPre(a.attribs.href, 'https://www.mof.gov.tw').replace('http:', 'https:') : addPre(findTag(findTag(findTag(findTag(sp, 'span')[0], 'strong')[0], 'span')[0], 'a')[0].attribs.href, 'https://www.mof.gov.tw').replace('http:', 'https:');
                            driveName = `${obj.name} ${obj.date}${PathExtname(url)}`;
                            console.log(driveName);
                            return mkFolder(PathDirname(filePath)).then(() => Api('url', url, {filePath}).then(() => GoogleApi('upload', {
                                type: 'auto',
