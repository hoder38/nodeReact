                        ...
                        const startStatus = () => {
                            return Mongo('find', TOTALDB, {_id: item._id}).then(nitem => {
                                ...
                                let checkMid = (item.newMid.length > 1) ? item.newMid[item.newMid.length - 2] : item.mid;
                                 while ((item.newMid.length > 0) && ((item.newMid[item.newMid.length - 1] > checkMid && (+priceData[item.index].lastPrice < checkMid || item.newMid[item.newMid.length - 1] <= item.mid || item.newMid[item.newMid.length - 1] <= item.web[item.web.length -1])) || (item.newMid[item.newMid.length - 1] <= checkMid && (+priceData[item.index].lastPrice > checkMid || item.newMid[item.newMid.length - 1] > item.mid || item.newMid[item.newMid.length - 1] > item.web[0])))) {
                                    item.newMid.pop();
